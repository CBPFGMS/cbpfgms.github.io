(function d3ChartIIFE() {
	...
	const isInternetExplorer = window.navigator.userAgent.indexOf("MSIE") > -1 || window.navigator.userAgent.indexOf("Trident") > -1,
		...
		cssLinks = ["https://cbpfgms.github.io/css/d3chartstyles.css", "https://cbpfgms.github.io/css/d3chartstylespbigam-stg.css", fontAwesomeLink],
	...
	function d3Chart() {
		...
		chartState.showMean = queryStringValues.has("showmean") ? queryStringValues.get("showmean") === "true" : containerDiv.node().getAttribute("data-showmean") === "true";

		chartState.display = queryStringValues.has("display") ? queryStringValues.get("display") :
			displayTypes.indexOf(containerDiv.node().getAttribute("data-display")) > -1 ? containerDiv.node().getAttribute("data-display") : displayTypes[0];

		chartState.allocationValue = queryStringValues.has("valuetype") ? queryStringValues.get("valuetype") :
			allocationValueTypes[containerDiv.node().getAttribute("data-valuetype").replace(" ", "").toLowerCase()] ? allocationValueTypes[containerDiv.node().getAttribute("data-valuetype").replace(" ", "").toLowerCase()] : "allocations";
		...
		const topDiv = containerDiv.append("div")
			.attr("class", "pbigamTopDiv");

		const titleDiv = topDiv.append("div")
			.attr("class", "pbigamTitleDiv");
		...
		const footerDiv = !isPfbiSite ? containerDiv.append("div")
			.attr("class", "pbigamFooterDiv") : null;
		...
		const legendPanel = {
			main: svg.append("g")
				...
				.attr("transform", "translate(" + padding[3] + "," + (padding[0] + topPanel.height + buttonsPanel.height + beeswarmPanel.height + (3 * panelHorizontalPadding)) + ")"),
			width: width - padding[1] - padding[3],
		...
		const yAxis = d3.axisLeft(yScale)
			...
			.tickFormat(function(d) {
				return chartState.display === "aggregated" ? "All" :
					chartState.gamGroup === "GM" && d === "3" ? "3/4" : d;
			});

		const xAxisGroup = beeswarmPanel.main.append("g")
			.attr("class", "pbigamxAxisGroup")
			.attr("transform", "translate(0," + (beeswarmPanel.padding[0] - beeswarmPanel.axisVerticalPadding) + ")");

		const yAxisGroup = beeswarmPanel.main.append("g")
		...
		const simulation = d3.forceSimulation()
			...
			.force("yPosition", d3.forceY(function(d) {
				return chartState.display === "aggregated" ? yScale(aggregatedDomain[0]) :
					yScale(d.gamCode) === undefined ? yScale("3") : yScale(d.gamCode);
			}).strength(yStrength))
		...
		function csvCallback(rawData, metaData) {
			...
			chartState.gamGroup = selectedGamGroup || (chartState.selectedYear.length === 1 ? yearsArray.filter(function(d) {
				return chartState.selectedYear.indexOf(d.year) > -1;
			}).reverse()[0].gamGroup : yearsArray.filter(function(d) {
		...
		function createTitle(rawData) {
			...
			downloadIcon.on("click", function() {
				...
				const fileName = "GenderAgeMarker_" + csvDateFormat(currentDate) + ".csv";
		...
		function createTopPanel(data) {
			...
			const topPanelMoneyBag = topPanel.main.selectAll(".pbigamtopPanelMoneyBag")
				.data([true])
				.enter()
				.append("g")
			...
			const previousValue = d3.select(".pbigamtopPanelMainValue").size() !== 0 ? d3.select(".pbigamtopPanelMainValue").datum() : 0;

			const previousProjects = d3.select(".pbigamtopPanelProjectsNumber").size() !== 0 ? d3.select(".pbigamtopPanelProjectsNumber").datum() : 0;

			const previousCbpfs = d3.select(".pbigamtopPanelCbpfsNumber").size() !== 0 ? d3.select(".pbigamtopPanelCbpfsNumber").datum() : 0;

			let mainValueGroup = topPanel.main.selectAll(".pbigammainValueGroup")
				.data([true]);
			...
			let topPanelMainValue = mainValueGroup.selectAll(".pbigamtopPanelMainValue")
				.data([mainValue]);
			...
			let topPanelSubText = mainValueGroup.selectAll(".pbigamtopPanelSubText")
				.data([true]);
			...
			let topPanelProjectsNumber = mainValueGroup.selectAll(".pbigamtopPanelProjectsNumber")
				.data([projectsValue]);

			topPanelProjectsNumber = topPanelProjectsNumber.enter()
				.append("text")
				.attr("class", "pbigamtopPanelProjectsNumber contributionColorFill")
				.attr("text-anchor", "end")
				.merge(topPanelProjectsNumber)
				.attr("y", topPanel.height - topPanel.mainValueVerPadding)
				.attr("x", topPanel.moneyBagPadding + topPanel.leftPadding[1] - topPanel.mainValueHorPadding);

			topPanelProjectsNumber.transition()
				.duration(duration)
				.tween("text", function(d) {
					const node = this;
			...
			let topPanelProjectsText = mainValueGroup.selectAll(".pbigamtopPanelProjectsText")
				.data([projectsValue]);

			topPanelProjectsText = topPanelProjectsText.enter()
				.append("text")
			...
			topPanelCbpfsNumber = topPanelCbpfsNumber.enter()
				.append("text")
				.attr("class", "pbigamtopPanelCbpfsNumber contributionColorFill")
				.attr("text-anchor", "end")
			...
			let topPanelCbpfsText = mainValueGroup.selectAll(".pbigamtopPanelCbpfsText")
				.data([cbpfsValue]);
		...
		function createButtonsPanel(rawData) {
			...
			const buttonsLegendTexts = buttonsLegendGroup.append("text")
			...
			const gmRect = buttonsGroup.append("rect")
				.attr("rx", "1px")
				.attr("ry", "1px")
				.attr("x", buttonsPanel.buttonPadding / 2)
				.attr("y", buttonsPanel.height - buttonsPanel.padding[2])
				.attr("height", 4)
				.attr("width", gmYears * buttonsPanel.buttonWidth - buttonsPanel.buttonPadding)
			...
			const leftArrowText = leftArrow.append("text")
				...
				.attr("x", 0)
				.attr("y", buttonsPanel.padding[0] + buttonsPanel.buttonVerticalPadding * 5.4)
				.style("fill", "#666")
				.text("\u25c4");
			...
			const buttonsAllocationsGroup = buttonsPanel.main.append("g")
				.attr("class", "pbigambuttonsAllocationsGroup")
				.style("cursor", "pointer");

			const buttonsAllocationsRects = buttonsAllocationsGroup.selectAll(null)
			...
			const buttonsAllocationsText = buttonsAllocationsGroup.selectAll(null)
				.data(d3.keys(allocationValueTypes))
				.enter()
				.append("text")
				.attr("text-anchor", "middle")
			...
			const buttonsAllocationsSize = buttonsAllocationsGroup.node().getBoundingClientRect().width;

			const allocationsPosition = buttonsGroupSize + (buttonsPanel.displayPadding - buttonsGroupSize - buttonsAllocationsSize) / 2;

			buttonsAllocationsGroup.attr("transform", "translate(" + allocationsPosition + ",0)");

			const buttonsDisplayGroup = buttonsPanel.main.append("g")
				.attr("class", "pbigambuttonsDisplayGroup")
				.attr("transform", "translate(" + buttonsPanel.displayPadding + ",0)")
				.style("cursor", "pointer");

			const buttonsDisplayRects = buttonsDisplayGroup.selectAll(null)
				.data(displayTypes)
				.enter()
				.append("rect")
				.attr("rx", "2px")
				...
				.attr("width", buttonsPanel.buttonDisplayWidth - buttonsPanel.buttonPadding)
				.attr("height", buttonsPanel.height - buttonsPanel.padding[0] - buttonsPanel.padding[2] - buttonsPanel.buttonVerticalPadding * 2)
				.attr("y", buttonsPanel.padding[0] + buttonsPanel.buttonVerticalPadding)
				.attr("x", function(_, i) {
					return i * buttonsPanel.buttonDisplayWidth + buttonsPanel.buttonPadding / 2;
				})
				.style("fill", function(d) {
					return d === chartState.display ? unBlue : "#eaeaea";
				});

			const buttonsDisplayText = buttonsDisplayGroup.selectAll(null)
				.data(displayTypes)
				.enter()
				.append("text")
				.attr("text-anchor", "middle")
				...
				.attr("y", 4.8 * buttonsPanel.buttonVerticalPadding)
				.attr("x", function(_, i) {
					return i * buttonsPanel.buttonDisplayWidth + buttonsPanel.buttonDisplayWidth / 2;
				})
				.style("fill", function(d) {
			...
			function clickButtonsRects(d, singleSelection) {
				...
				buttonsRects.style("fill", function(e) {
					return chartState.selectedYear.indexOf(e.year) > -1 && chartState.gamGroup === e.gamGroup ? unBlue : "#eaeaea";
				});

				buttonsText.style("fill", function(e) {
					return chartState.selectedYear.indexOf(e.year) > -1 && chartState.gamGroup === e.gamGroup ? "white" : "#444";
				});
			...
			function mouseOverButtonsAllocationsRects(d) {
				...
				tooltip.style("top", thisBoundingRect.top - containerBoundingRect.top - tooltipBoundingRect.height - 8 + "px")
					.style("left", thisBoundingRect.left - containerBoundingRect.left + thisBoundingRect.width / 2 - tooltipBoundingRect.width / 2 + "px");
			...
			function mouseOutButtonsDisplayRects(d) {
				...
				d3.select(this).style("fill", "#eaeaea");
				buttonsDisplayText.filter(function(e) {
						return e === d
					})
					.style("fill", "#444");
			};
			...
			function mouseOutButtonsAllocationsRects(d) {
				tooltip.style("display", "none");
				if (allocationValueTypes[d] === chartState.allocationValue) return;
				d3.select(this).style("fill", "#eaeaea");
				buttonsAllocationsText.filter(function(e) {
						return e === d
					})
					.style("fill", "#444");
			};
			...
			function clickButtonsAllocationsRects(d) {
				...
				buttonsAllocationsRects.style("fill", function(e) {
					return chartState.allocationValue === allocationValueTypes[e] ? unBlue : "#eaeaea";
				});
		...
		function createBeeswarm(data) {
			...
			radiusScale.domain([1, d3.max(data, function(d) {
				return d.projects;
			})]);
			...
			const meanData = yScale.domain().map(function(d) {
				return {
					...
					mean: d === "aggregated" ?
						...
						}) : d3.mean(data.filter(function(e) {
							return e.gamCode === d;
						}), function(e) {
							return e[chartState.allocationValue];
						})
			...
			let beeswarm = beeswarmPanel.main.selectAll(".pbigambeeswarm")
				.data(data, function(d) {
					return d.cbpfId + d.keygamGroup + d.gamCode;
				});

			const beeswarmExit = beeswarm.exit()
				.transition()
				.duration(duration)
				.style("opacity", 0)
				.remove();

			const beeswarmEnter = beeswarm.enter()
				.append("circle")
			...
			xAxis.tickSizeInner(-(beeswarmPanel.height - beeswarmPanel.padding[0] - beeswarmPanel.padding[2] + beeswarmPanel.axisVerticalPadding))
				.tickFormat(function(d) {
					return chartState.allocationValue === "allocations" ? formatSIaxes(d).replace("G", "B") : formatPercent(d);
				});
			...
			yAxis.tickSizeInner(chartState.display !== "aggregated" ?
			...
			if (chartState.display !== "aggregated") {
				gamDescriptionData = JSON.parse(JSON.stringify(gamDescriptions.filter(function(d) {
					return chartState.gamGroup === d.gamGroup;
				})));
				if (chartState.gamGroup === "GM") {
					const lastValidObject = gamDescriptionData.find(function(d) {
						return d.gamCode === "3"
					});
					const lastObject = gamDescriptionData.find(function(d) {
						return d.gamCode === "4"
					});
					lastValidObject.gamDescription = lastValidObject.gamDescription + "/" + lastObject.gamDescription;
					gamDescriptionData = gamDescriptionData.filter(function(d) {
						return d.gamCode !== "4";
					});
				};
			...
			let axisDescriptions = beeswarmPanel.main.selectAll(".pbigamaxisDescriptions")
				.data(gamDescriptionData);

			const axisDescriptionsExit = axisDescriptions.exit()
				.transition()
				.duration(duration)
				.style("opacity", 0)
				.remove();

			const axisDescriptionsEnter = axisDescriptions.enter()
				.append("text")
				.attr("class", "pbigamaxisDescriptions")
				.attr("text-anchor", "middle")
				.attr("x", (beeswarmPanel.padding[3] - axisDescriptionsPadding) / 2);

			axisDescriptions = axisDescriptionsEnter.merge(axisDescriptions);

			axisDescriptions.attr("y", function(d) {
					return yScale(d.gamCode) || yScale("aggregated");
				})
				.text(function(d) {
					return d.gamDescription || d;
				})
				.call(wrapText, chartState.display !== "aggregated" ?
					(beeswarmPanel.padding[3] - axisDescriptionsPadding) : (beeswarmPanel.padding[3] - axisDescriptionsPadding - 30));
			...
			axisDescriptions.each(function(d) {
				...
				d3.select(this)
					...
					.attr("y", numberOfLines - 1 ?
						currentValue - (axisDescriptionsLineHeight * (numberOfLines - 1)) : currentValue + 3)
			});

			let meanGroup = beeswarmPanel.main.selectAll(".pbigammeanGroup")
				.data(meanData, function(d) {
					return d.gamCode;
				});

			const meanGroupExit = meanGroup.exit()
				.transition()
				.duration(duration)
				.style("opacity", 0)
				.remove();

			const meanGroupEnter = meanGroup.enter()
				.append("g")
				.attr("class", "pbigammeanGroup")
				.attr("transform", function(d) {
					return "translate(" + xScale(d.mean) + "," + yScale(d.gamCode) + ")";
				});

			meanGroupEnter.append("line")
			...
			meanGroup.select(".pbigammeanGroupTextBack")
				.text(function(d) {
					return "mean: " + (chartState.allocationValue === "allocations" ? formatSIFloat(d.mean) : formatPercent(d.mean));
				});

			meanGroup.select(".pbigammeanGroupText")
				.text(function(d) {
					return "mean: " + (chartState.allocationValue === "allocations" ? formatSIFloat(d.mean) : formatPercent(d.mean));
				});
		...
		function createLegend(data) {
			...
			const legendSizeTitle = legendPanel.main.selectAll(".pbigamlegendSizeTitle")
				.data([true])
				.enter()
				.append("text")
				.attr("text-anchor", "end")
			...
			const legendSizeData = [projectsMaxValue, projectsMaxValue / Math.PI, 1];

			const legendSizeCircles = legendPanel.main.selectAll(".pbigamlegendSizeCircles")
				.data(legendSizeData)
				.enter()
				.append("circle")
			...
			const legendSizeLines = legendPanel.main.selectAll(".pbigamlegendSizeLines")
				.data(legendSizeData)
				.enter()
				.append("line")
				.attr("class", "pbigamlegendSizeLines")
				.attr("x1", beeswarmPanel.padding[3] + maxRadius)
				.attr("x2", beeswarmPanel.padding[3] + maxRadius + legendSizeLine)
				.attr("y1", function(d) {
			...
			let legendSizeLabels = legendPanel.main.selectAll(".pbigamlegendSizeLabels")
				.data(legendSizeData);

			legendSizeLabels = legendSizeLabels.enter()
				.append("text")
				.attr("class", "pbigamlegendSizeLabels")
				.attr("x", beeswarmPanel.padding[3] + maxRadius + legendSizeLine + 2)
				.attr("y", function(d) {
					return legendPanel.height + 1 - radiusScale(d) * 2;
				})
				.merge(legendSizeLabels);

			legendSizeLabels.transition()
				.duration(duration)
				.tween("text", function(d) {
					const node = this;
			...
			const legendColorTitle = legendPanel.main.selectAll(".pbigamlegendColorTitle")
				.data([true])
				.enter()
				.append("text")
			...
			legendPanel.main.select(".pbigamlegendColorTitle")
				.transition()
				.duration(duration)
				.style("opacity", chartState.display === "aggregated" ? 1 : 0);

			const legendColorData = chartState.display !== "aggregated" ? [] :
				chartState.gamGroup === "GM" ? gmDomain : gamDomain;

			let legendColorGroup = legendPanel.main.selectAll(".pbigamlegendColorGroup")
				.data(legendColorData);

			const legendColorGroupExit = legendColorGroup.exit()
				.transition()
				.duration(duration)
				.style("opacity", 0)
				.remove();

			const legendColorGroupEnter = legendColorGroup.enter()
				.append("g")
				.attr("class", "pbigamlegendColorGroup")
				.attr("transform", function(_, i) {
					return "translate(" + (beeswarmPanel.padding[3] + legendColorPadding + 8 + i * 38) + ",0)";
				});

			legendColorGroupEnter.append("rect")
				.attr("y", 15)
				.attr("width", buttonsLegendRectSize)
				.attr("height", buttonsLegendRectSize)
				.attr("rx", 2)
			...
			legendColorGroupEnter.append("text")
				.attr("y", 25)
				.attr("x", buttonsLegendRectSize + 3)
				.text(function(d) {
			...
			legendColorGroup.select("rect")
				.style("fill", function(d) {
					return colorsScale(d);
				});

			legendColorGroup.select("text")
				.text(function(d) {
					return chartState.gamGroup === "GM" && d === "3" ? "3/4" : d;
				});

			legendColorGroup.on("mouseover", function(d) {
				beeswarmPanel.main.selectAll(".pbigambeeswarm")
			...
			let showMeanGroup = legendPanel.main.selectAll(".pbigamshowMeanGroup")
				.data([{
					clicked: chartState.showMean
				}]);

			const showMeanGroupEnter = showMeanGroup.enter()
				.append("g")
				.attr("class", "pbigamshowMeanGroup")
				.attr("cursor", "pointer")
				.attr("transform", "translate(" + (legendPanel.width - showMeanPadding) + "," + (legendPanel.height / 2 - 1) + ")");

			showMeanGroupEnter.append("rect")
				.attr("width", 12)
				.attr("height", 12)
				.attr("rx", 2)
				.attr("ry", 2)
			...
			showMeanGroupEnter.append("polyline")
				.style("stroke-width", "2px")
				.attr("points", "-4,1 -1,4 4,-3")
				.style("fill", "none")
				.style("stroke", chartState.showMean ? "darkslategray" : "white");

			showMeanGroupEnter.append("text")
			...
			showMeanGroup.on("click", function(d) {
				...
				showMeanGroup.select("polyline")
					.style("stroke", chartState.showMean ? "darkslategray" : "white");
				beeswarmPanel.main.selectAll(".pbigammeanGroup")
		...
		function resizeSVG() {
			...
			const numberOfGroups = chartState.display === "aggregated" ? aggregatedDomain.length :
				chartState.gamGroup === "GM" ? gmDomain.length : gamDomain.length;

			const newBeeswarmGroupHeight = chartState.display === "aggregated" ? beeswarmGroupHeight * 1.5 : beeswarmGroupHeight;

			beeswarmPanel.height = beeswarmPanel.padding[0] + beeswarmPanel.padding[2] + numberOfGroups * newBeeswarmGroupHeight;

			height = padding[0] + padding[2] + topPanel.height + buttonsPanel.height + beeswarmPanel.height + legendPanel.height + (3 * panelHorizontalPadding);
		...
		function setScales() {
			...
			colorsScale.domain(chartState.gamGroup === "GM" ? gmDomain : gamDomain);

			yScale.domain(chartState.display === "aggregated" ? aggregatedDomain :
					chartState.gamGroup === "GM" ? gmDomain : gamDomain)
		...
		function processRawData(rawData, metaData) {
			...
			rawData.forEach(function(row) {
				...
				const foundYearAndGamGroup = yearsArray.find(function(d) {
					return d.year === +row.AllocationYear && d.gamGroup === row.GenderMarkerGroup;
				});

				if (!foundYearAndGamGroup) yearsArray.push({
		...
		function processData(rawData) {
			...
			rawData.forEach(function(row) {
				...
				if (chartState.selectedYear.indexOf(+row.AllocationYear) > -1 && chartState.gamGroup === row.GenderMarkerGroup && chartState.cbpfsInData.indexOf("id" + row.PooledFundId) === -1) {
					chartState.cbpfsInData.push("id" + row.PooledFundId);
				};

				if (chartState.selectedYear.indexOf(+row.AllocationYear) > -1 && chartState.selectedCbpfs.indexOf("id" + row.PooledFundId) > -1 && chartState.gamGroup === row.GenderMarkerGroup) {

					const foundCbpf = data.find(function(d) {
						return d.cbpfId === "id" + row.PooledFundId && d.gamCode === row.GenderMarkerCode && d.keygamGroup === row.GenderMarkerGroup;
					});
					...
					if (foundGamCounter) foundGamCounter.total += +row.TotalBudget;
					const foundCbpfsCounter = cbpfsCounter.find(function(d) {
						return d.cbpfId === "id" + row.PooledFundId;
					});
					if (foundCbpfsCounter) foundCbpfsCounter.total += +row.TotalBudget;
				};
			...
			data.forEach(function(row) {
				...
				row.percentageGam = thisGamTotal && thisGamTotal.total ? row.allocations / thisGamTotal.total : 0;
				row.percentageCbpf = thisCbpfTotal && thisCbpfTotal.total ? row.allocations / thisCbpfTotal.total : 0;
			});
		...
		function validateYear(yearString) {
			...
			const allYears = yearString.split(",").map(function(d) {
				...
				return d && yearsArray.find(function(e) {
					return e.year === d;
				});
			}).sort(function(a, b) {
				return a - b;
			...
			allYears.forEach(function(d) {
				if (d !== yearWithMixedGroups) {
					...
					if (gamGroups.indexOf(yearGroup) === -1) gamGroups.push(yearGroup)
				};
			});
		...
		function downloadSnapshotPng(source) {
			...
			const fileName = "GenderAgeMarker_" + csvDateFormat(currentDate) + ".png";
		...
		function downloadSnapshotPdf(source) {
			...
			d3.image("https://raw.githubusercontent.com/CBPFGMS/cbpfgms.github.io/master/img/assets/bilogo.png")
				.then(function(logo) {
					...
					pdf.save("GenderAgeMarker__" + csvDateFormat(currentDate) + ".pdf");
