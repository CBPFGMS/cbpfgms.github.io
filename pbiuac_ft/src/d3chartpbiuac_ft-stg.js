(function d3ChartIIFE() {
	...
	function d3Chart() {
		...
		const xScaleCopy = xScaleMain.copy();
		...
		function csvCallback(rawData) {
			...
			xScaleCopy.domain(xScaleMain.domain());
			...
			function mouseOverLegend(datum, scale, element, modality) {
				...
				innerTooltip.html(() => {
					if (!limits[0] || !limits[1]) {
						return "Values for this allocation type and bracket don't exist in the data";
					}
					return (
		...
		function zoomed() {
			...
			xScaleMain.domain(t.rescaleX(xScaleCopy).domain());
		...
		function calculateHeightandResize() {
			...
			mainPanel.main.attr(
				...
				"translate(" +
					padding[3] +
					"," +
					(padding[0] + topPanel.height + panelHorizontalPadding) +
					")",
			);
		...
		function rewriteXAxisText() {
			...
			mainPanel.main
				...
				.each(function () {
					...
					if (
						thisSize.left < todaySize.right + 4 &&
						thisSize.right > todaySize.left - 4
					) {
