(function d3ChartIIFE() {
	const isInternetExplorer =
		...
		cssLinks = [
			...
			"https://cbpfgms.github.io/css/d3chartstylescbsank-stg.css",
			fontAwesomeLink,
		],
		d3URL = "https://cdnjs.cloudflare.com/ajax/libs/d3/5.16.0/d3.min.js",
		d3SankeyUrl =
			"https://cdnjs.cloudflare.com/ajax/libs/d3-sankey/0.12.3/d3-sankey.js",
		html2ToCanvas =
	...
	function d3Chart() {
		...
		function fetchCallback([
			...
			chartState.selectedFund =
				populateSelectedFunds(selectedFundsString);
		...
		function draw(
			...
			const dataAllocations = processDataAllocations(
				rawDataLaunchedAllocations,
			);
		...
		function createTitle(rawDataLaunchedAllocations, rawDataContributions) {
			...
			helpIcon.html("HELP  ").append("span").attr("class", "fa fa-info");
			...
			downloadIcon.on("click", function () {
				const csvAllocations = createCsvAllocations(
					rawDataLaunchedAllocations,
				);
				...
				if (navigator.msSaveBlob) {
					...
					if (linkAllocations.download !== undefined) {
						const urlAllocations =
							URL.createObjectURL(blobAllocations);
		...
		function createCheckboxes(
			...
			checkboxData.sort(function (a, b) {
				return lists.fundsInAllYears[a].localeCompare(
					lists.fundsInAllYears[b],
				);
			});
				...
				.property("checked", function (d) {
					return (
						chartState.selectedFund.length !==
							lists.fundsInAllYearsKeys.length &&
						chartState.selectedFund.indexOf(+d) > -1
					);
				})
			...
			d3.select(allCbpfs.node().nextSibling).attr(
				"class",
				classPrefix + "CheckboxTextAllCbpfs",
			);
			...
			checkbox.select("input").on("change", function () {
				if (this.value === "All CBPFs") {
					if (this.checked) {
						chartState.selectedFund =
							lists.fundsInAllYearsKeys.slice();
					...
					if (this.checked) {
						...
						const thisIndex = chartState.selectedFund.indexOf(
							+this.value,
						);
				...
				if (
					...
					const allFunds = chartState.selectedFund
						.map(function (d) {
		...
		function createButtonsPanel(
			...
			function clickButtonsRects(d, singleSelection) {
				...
				selectDiv
					.selectAll("." + classPrefix + "CheckboxDiv")
					.filter(function (d) {
						return d !== "All CBPFs";
					})
					.select("input")
		...
		function drawCentralCircle(dataContributions, dataAllocations) {
			...
			contributionsSubText = contributionsSubText
				...
				.attr("y", -centralCirclePanel.radius * 0.1)
			...
			let launchedAllocationsValueText = centralCirclePanel.main
				.selectAll("." + classPrefix + "launchedAllocationsValueText")
				.data([dataAllocations.launchedAllocations]);

			const launchedAllocationsValueTextEnter =
				launchedAllocationsValueText
					.enter()
					.append("text")
					...
					.attr(
						"class",
						classPrefix + "launchedAllocationsValueTextSpan",
					)
					.transition()
					.duration(duration)
					.tween("text", (d, i, n) => {
			...
			let launchedAllocationsText = centralCirclePanel.main
				.selectAll("." + classPrefix + "launchedAllocationsText")
				.data([true]);

			launchedAllocationsText = launchedAllocationsText
				.enter()
				.append("text")
			...
			let selectedFundsText = centralCirclePanel.main
				.selectAll("." + classPrefix + "selectedFundsText")
				.data([true]);

			selectedFundsText = selectedFundsText
				.enter()
				.append("text")
				...
				.attr(
					"x",
					centralCirclePanel.radius +
						centralCirclePanel.selectedFundsPadding,
				)
			...
			const fundsList =
				...
				lists.fundsInAllYearsKeys.length
					...
					: chartState.selectedFund
							.sort((a, b) =>
								lists.fundsInAllYears[a].localeCompare(
									lists.fundsInAllYears[b],
								),
							)
							.reduce(
								(acc, curr, index) =>
									acc +
									(index >= chartState.selectedFund.length - 2
										? index >
											chartState.selectedFund.length - 2
											? lists.fundsInAllYears[curr]
											: lists.fundsInAllYears[curr] +
												" and "
										: lists.fundsInAllYears[curr] + ", "),
								"",
			...
			selectedFundsText
				.text(
					"Selected fund" +
						(chartState.selectedFund.length === 1 ? "" : "s") +
						": " +
						fundsList,
				)
				.call(
		...
		function drawSankeyContributions(dataContributions, flagsData) {
			const nonZeroData =
				dataContributions.nodes.length &&
				d3.sum(dataContributions.nodes, d => d.value);
			...
			if (!nonZeroData) {
				contributionsPanel.main
					.selectAll("*:not(." + classPrefix + "contributionsNoData)")
					.remove();
				return;
			}
			...
			const sankeyDonorFlagsEnter = sankeyDonorFlags
				...
				.attr(
					"y",
					contributionsPanel.padding[0] - flagPadding - flagSize,
				)
				.attr("width", flagSize)
			...
			const otherFlagsEnter = otherFlags
				...
				.attr(
					...
					(inverseContributionsScale(
						!donorNodes.length ? 0 : donorNodes[0].y1,
					) +
						inverseContributionsScale(
							!donorNodes.length ? 0 : donorNodes[0].y0,
						)) /
						2 -
						flagSize / 2,
				)
			...
			const sankeyDonorNamesEnter = sankeyDonorNames
				...
				.attr(
					"transform",
					d =>
						`rotate(${angle}, ${(inverseContributionsScale(d.y1) + inverseContributionsScale(d.y0)) / 2}, ${contributionsPanel.padding[0] - donorNamesPadding - flagSize - flagPadding})`,
				)
				.text(d =>
					d.codeId === othersId
						? "Others"
						: lists.donorNames[d.codeId],
				)
			...
			const sankeyDonorValuesEnter = sankeyDonorValues
				...
				.attr(
					"transform",
					d =>
						`rotate(${angleValues}, ${(inverseContributionsScale(d.y1) + inverseContributionsScale(d.y0)) / 2}, ${d.x1 + valuesPaddingDonors})`,
				)
				.text(d => "$0");
		...
		function drawSankeyAllocations(dataAllocations) {
			const nonZeroData =
				dataAllocations.nodes.length &&
				d3.sum(dataAllocations.nodes, d => d.value);
			...
			if (!nonZeroData) {
				allocationsPanel.main
					.selectAll("*:not(." + classPrefix + "allocationsNoData)")
					.remove();
				return;
			}
			...
			const sankeyDataAllocations =
				sankeyGeneratorAllocations(dataAllocations);

			const fundNode = sankeyDataAllocations.nodes.filter(
				e => e.level === 1,
			);
			const typesNodes = sankeyDataAllocations.nodes.filter(
				e => e.level === 2,
			);
			const fundsNodes = sankeyDataAllocations.nodes.filter(
				e => e.level === 3,
			);
			...
			clusterColorsScale
				.domain(fundsNodes.map(d => d.id).reverse())
				.range(
					d3
						.range(fundsNodes.length)
						.map(
							d =>
								clusterColorsArray[
									~~(
			...
			sankeyPartnerNames =
				sankeyPartnerNamesEnter.merge(sankeyPartnerNames);
			...
			const sankeyClusterNamesEnter = sankeyClusterNames
				...
				.attr(
					"transform",
					d =>
						`rotate(${angle}, ${(inverseAllocationsScale(d.y1) + inverseAllocationsScale(d.y0)) / 2}, ${d.x0 - namesPadding})`,
				)
				.text(d => lists.fundAbbreviatedNames[d.codeId])
				.call(createEllipsis, "cluster");

			sankeyClusterNames =
				sankeyClusterNamesEnter.merge(sankeyClusterNames);
			...
			const sankeyClusterValuesEnter = sankeyClusterValues
				...
				.attr(
					"transform",
					d =>
						`rotate(${angleValues}, ${(inverseAllocationsScale(d.y1) + inverseAllocationsScale(d.y0)) / 2}, ${d.x1 + valuesPaddingDonors})`,
				)
				.text(d => "$0");
			...
			function mouseOverPartnerNodes(datum) {
				...
				sankeyNodesAllocations.style("opacity", d =>
					d.level === 2 && d.id === datum.id
						? 1
						: d.targetLinks.filter(e => e.source.id === datum.id)
									.length
							? 1
							: fadeOpacityNodes,
				);
				...
				sankeyClusterValues.each((d, i, n) => {
					const amountFromPartner = d.targetLinks.reduce(
						(acc, curr) => {
			...
			function mouseOverClusterNodes(datum) {
				...
				sankeyNodesAllocations.style("opacity", d =>
					d.level === 3 && d.id === datum.id
						? 1
						: d.sourceLinks.filter(e => e.target.id === datum.id)
									.length
							? 1
							: fadeOpacityNodes,
				);
				...
				sankeyLinksAllocations.style("stroke-opacity", d =>
					d.target.id === datum.id ||
					linksToCluster.includes(d.target.id)
						? linksOpacity
						: fadeOpacityLinks,
				);
				sankeyPartnerNames.style("opacity", d =>
				...
				sankeyPartnerValues.each((d, i, n) => {
					const amountFromCluster = d.sourceLinks.reduce(
						(acc, curr) => {
			...
			function mouseOverPartnerLinks(datum) {
				...
				sankeyClusterValues.each((d, i, n) => {
					const amountFromPartner = d.targetLinks.reduce(
						(acc, curr) => {
							if (curr.source.id === datum.target.id)
								acc += curr.value;
							return acc;
						},
						0,
			...
			function mouseOverClusterLinks(datum) {
				...
				sankeyPartnerValues.each((d, i, n) => {
					const amountFromCluster = d.sourceLinks.reduce(
						(acc, curr) => {
							if (curr.target.id === datum.target.id)
								acc += curr.value;
							return acc;
						},
						0,
				...
				sankeyClusterValues.each((d, i, n) => {
					const amountFromPartner = d.targetLinks.reduce(
						(acc, curr) => {
							if (curr.source.id === datum.source.id)
								acc += curr.value;
							return acc;
						},
						0,
			...
			function generateClusterTooltip(datum, type) {
				...
				titleNameDiv
					...
					.html(
						type === "node"
							? lists.fundAbbreviatedNames[datum.codeId]
							: lists.fundAbbreviatedNames[datum.target.codeId],
					);
				...
				if (type === "link") {
					titleNameDiv
						...
						.html(
							"(from: " +
								lists.allocationTypes[datum.source.codeId] +
								")",
						);
				}
		...
		function preProcessData(
			...
			rawDataLaunchedAllocations.forEach(row => {
				...
				if (
					!yearsArrayAllocations.includes(row.AllocationYear) &&
					+row.fundId === +row.fundId
				)
					yearsArrayAllocations.push(row.AllocationYear);
				if (
					!lists.fundsInAllYears[row.fundId] &&
					+row.fundId === +row.fundId &&
					+row.AllocationYear >= firstYear
				)
					lists.fundsInAllYears[row.fundId] =
						lists.fundAbbreviatedNames[row.fundId];
			});

			rawDataContributions.forEach(row => {
				//SECOND CONDITION IS FOR REMOVING CERF
				if (
					!yearsArrayContributions.includes(row.contributionYear) &&
					row.fundId !== cerfPooledFundId &&
					+row.fundId === +row.fundId
				)
					yearsArrayContributions.push(row.contributionYear);
				//SECOND CONDITION IS FOR REMOVING CERF
				if (
					!lists.fundsInAllYears[row.fundId] &&
					row.fundId !== cerfPooledFundId &&
					+row.fundId === +row.fundId &&
					+row.contributionYear >= firstYear
				)
					lists.fundsInAllYears[row.fundId] =
						lists.fundAbbreviatedNames[row.fundId];
			});

			lists.fundsInAllYearsKeys = Object.keys(lists.fundsInAllYears).map(
				d => +d,
			);
			...
			yearsArrayAllocations.forEach(e => {
				...
				if (yearsArrayContributions.includes(e) && e >= firstYear)
					yearsArray.push(e);
			});
		}
			...
			rawDataLaunchedAllocations.forEach(row => {
				...
				if (row.FundId !== 1) {
					...
					if (
						...
						const valueColumn = yearsWithUnderApprovalAboveMin[
							row.AllocationYear
						]
							? "TotalUSDPlanned"
							: "TotalApprovedBudget";
						...
						const foundType = data.nodes.find(
							d =>
								d.level === 2 &&
								d.codeId ===
									lists.allocationTypesReversed[
										row.AllocationSource.toLowerCase()
									],
						);

						const foundFund = data.nodes.find(
							d => d.level === 3 && d.codeId === row.fundId,
						);
						...
						if (foundType) {
							...
							data.nodes.push({
								codeId: lists.allocationTypesReversed[
									row.AllocationSource.toLowerCase()
								],
								level: 2,
						...
						const foundLinkToType = data.links.find(
							d =>
								d.target ===
								"alloctype#" +
									lists.allocationTypesReversed[
						...
						const foundLinkToFund = data.links.find(
							d =>
								d.source ===
									"alloctype#" +
										lists.allocationTypesReversed[
											row.AllocationSource.toLowerCase()
										] && d.target === "fund#" + row.fundId,
						);
		...
		function createFundNamesList(fundsData) {
			fundsData.forEach(row => {
				...
				lists.fundAbbreviatedNames[row.id + ""] =
					row.PooledFundNameAbbrv;
		...
		function createCbpfIdsList(fundsData) {
			fundsData.forEach(row => {
				if (row.CBPFId && !row.PooledFundName.includes("Closed"))
		...
		function createAllocationTypesList(allocationTypesData) {
			allocationTypesData.forEach(row => {
				...
				lists.allocationTypesReversed[
					row.AllocationName.toLowerCase()
				] = row.id;
			});
		}
		...
		function populateSelectedFunds(fundsString) {
			...
			const someInvalidValue = dataArray.some(function (d) {
				return (
					valuesInLowerCase(lists.fundsInAllYearsValues).indexOf(
						d,
					) === -1
				);
			});
			...
			dataArray.forEach(function (d) {
				lists.fundsInAllYearsKeys.forEach(key => {
					if (lists.fundsInAllYears[key].toLowerCase() === d)
						funds.push(key);
				});
			});
		...
		function createAnnotationsDiv() {
			...
			const helpData = [
				...
				},
				{
					...
					xTooltip: 300 * (topDivSize.width / width),
					yTooltip:
						(topDivHeight + totalSelectHeight + 50) *
						(topDivSize.width / width),
				...
				},
				{
					...
					xTooltip: 300 * (topDivSize.width / width),
					yTooltip:
						(topDivHeight + totalSelectHeight + 250) *
						(topDivSize.width / width),
				...
				},
				{
					...
					xTooltip: 300 * (topDivSize.width / width),
					yTooltip:
						(topDivHeight + totalSelectHeight + 500) *
						(topDivSize.width / width),
			...
			function createTooltip(xPos, yPos, text, self) {
				...
				helpSVG
					.selectAll("." + classPrefix + "HelpRectangle")
					.style("opacity", 0.1);
		...
		function verticallyCenter(selection) {
			...
			selection
				...
				.attr(
					"transform",
					"translate(0," +
						(linesNumber === 1 ? 0 : -textHeight / 2 + padding) +
						")",
				);
		}
