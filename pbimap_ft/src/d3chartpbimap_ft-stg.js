(function d3ChartIIFE() {
	const isInternetExplorer =
		...
		cssLinks = [
			...
			"https://cbpfgms.github.io/css/d3chartstylespbimap_ft-stg.css",
			fontAwesomeLink,
			leafletCSSLink,
		],
	...
	function d3Chart() {
		...
		Object.entries(rawCountryBoundingBoxes).forEach(
			([code, countryInfo]) => {
				const [name, bbox] = countryInfo;
				const [west, south, east, north] = bbox;

				if (!countryBoundingBoxes[name]) {
					countryBoundingBoxes[name] = {
						sw: {
							lat: south,
							lng: west,
						},
