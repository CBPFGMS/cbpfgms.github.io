(function d3ChartIIFE() {
	const isInternetExplorer =
		...
		cssLinks = [
			...
			"https://cbpfgms.github.io/css/d3chartstylespbiobe-stg.css",
			fontAwesomeLink,
		],
	...
	function d3Chart() {
		...
		const width = 900,
			...
			helpPortalUrl =
				"https://gms.unocha.org/content/targeted-and-reached-people",
		...
		const topDiv = containerDiv.append("div").attr("class", "pbiobeTopDiv");

		const titleDiv = topDiv.append("div").attr("class", "pbiobeTitleDiv");
		...
		const bottomGroup = svg.append("g").attr("class", "pbiobeBottomGroup");

		const xScalePercentage = d3
			.scaleLinear()
			.range([
				percentagePanel.padding[3],
				percentagePanel.width - percentagePanel.padding[1],
			]);

		const xScalePictograms = d3
			.scaleLinear()
			.range([
				pictogramsPanel.padding[3],
				pictogramsPanel.width - pictogramsPanel.padding[1],
			]);

		const yScalePercentage = d3
			.scaleBand()
			.range([
				percentagePanel.padding[0],
				percentagePanel.height - percentagePanel.padding[2],
			])
			.domain(beneficiariesTypes)
			.paddingInner(0.4)
			.paddingOuter(0.2);

		const yScalePictograms = d3
			.scalePoint()
			.range([
				percentagePanel.padding[0],
				percentagePanel.height - percentagePanel.padding[2],
		...
		const xAxisPercentage = d3
			.axisTop(xScalePercentage)
			.tickSizeOuter(0)
			.tickSizeInner(
				-(
					percentagePanel.height -
					percentagePanel.padding[0] -
					percentagePanel.padding[2]
				)
			)
			.tickValues([0, 0.25, 0.5, 0.75, 1])
			.tickFormat(formatPercent);

		const yAxisPercentage = d3
			.axisLeft(yScalePercentage)
		...
		if (isPfbiSite && !isBookmarkPage) {
			window.cbpfbiDataObject.targetedPersonsDetailsData.then(rawData =>
				csvCallback(rawData)
			);
		} else {
			if (
				localStorage.getItem("pbiobedata") &&
				JSON.parse(localStorage.getItem("pbiobedata")).timestamp >
					currentDate.getTime() - localStorageTime
			) {
				const rawData = d3.csvParse(
					JSON.parse(localStorage.getItem("pbiobedata")).data
				);
				console.info("pbiobe: data from local storage");
				csvCallback(rawData);
			} else {
				d3.csv(
					"https://cbpfapi.unocha.org/vo2/odata/ProjectSummaryBeneficiaryDetail?$format=csv&ShowAllPooledFunds=1"
		...
		function draw(rawData) {
					...
					function loopButtons() {
						...
						if (yearsArray.length > buttonsNumber) {
							...
							if (currentTranslate === 0) {
								svg.select(".pbiobeLeftArrowGroup")
									.select("text")
									.style("fill", "#ccc");
								svg.select(".pbiobeLeftArrowGroup").attr(
									"pointer-events",
									"none"
								);
							} else {
							...
							if (
								...
								svg.select(".pbiobeRightArrowGroup")
									.select("text")
									.style("fill", "#ccc");
								svg.select(".pbiobeRightArrowGroup").attr(
									"pointer-events",
									"none"
								);
							} else {
			...
			function createAxes() {
				const groupXAxisPercentage = percentagePanel.main
					.append("g")
					.attr("class", "pbiobegroupXAxisPercentage")
					.attr(
						"transform",
						"translate(0," + percentagePanel.padding[0] + ")"
					)
					.call(xAxisPercentage);

				const groupYAxisPercentage = percentagePanel.main
					.append("g")
					.attr("class", "pbiobegroupYAxisPercentage")
					.attr(
						"transform",
						"translate(" + percentagePanel.padding[3] + ",0)"
					)
					.call(yAxisPercentage);

				groupYAxisPercentage
					.selectAll(".tick")
					.filter(function (d) {
						return d === "total";
					})
			...
			function createTopPanel(data, totalClosedReports) {
				...
				const topPanelPictogram = topPanel.main
					...
					.attr(
						"class",
						"pbiobetopPanelPictogram contributionColorFill"
					)
					.attr(
						"transform",
						"translate(" +
							topPanel.pictogramPadding +
							",2) scale(2)"
					)
					.each(function (_, i, n) {
						["bodyDAttribute", "headDAttribute"].forEach(function (
							d
						) {
							d3.select(n[i])
								.append("path")
				...
				const previousValue =
					d3.select(".pbiobetopPanelMainValue").size() !== 0
						? d3.select(".pbiobetopPanelMainValue").datum()
						: 0;

				const previousActual =
					d3.select(".pbiobetopPanelPersonsActual").size() !== 0
						? d3.select(".pbiobetopPanelPersonsActual").datum()
						: 0;

				let mainValueGroup = topPanel.main
					.selectAll(".pbiobemainValueGroup")
					.data([true]);
				...
				let topPanelMainValue = mainValueGroup
					.selectAll(".pbiobetopPanelMainValue")
					.data([mainValue]);
				...
				topPanelMainValue = topPanelMainValue
					...
					.attr(
						"class",
						"pbiobetopPanelMainValue contributionColorFill"
					)
				...
				let topPanelMainText = mainValueGroup
					.selectAll(".pbiobetopPanelMainText")
					.data([mainValue]);
				...
				let topPanelPersonsActual = mainValueGroup
					.selectAll(".pbiobetopPanelPersonsActual")
					.data([actualValue]);

				topPanelPersonsActual = topPanelPersonsActual
					.enter()
					.append("text")
					.attr(
						"class",
						"pbiobetopPanelPersonsActual contributionColorFill"
					)
					.attr("text-anchor", "end")
				...
				let topPanelPersonsText = mainValueGroup
					.selectAll(".pbiobetopPanelPersonsText")
					.data([actualValue]);
			...
			function createPercentagePanel(data) {
				const title = percentagePanel.main
					.selectAll(".pbiobePercentagePanelTitle")
					.data([true])
					.enter()
					.append("text")
				...
				let beneficiaryGroup = percentagePanel.main
					.selectAll(".pbiobeBeneficiaryGroup")
					.data(data, function (d) {
						return d.beneficiary;
					});

				const beneficiaryGroupExit = beneficiaryGroup.exit().remove();

				const beneficiaryGroupEnter = beneficiaryGroup
					.enter()
					.append("g")
					.attr("class", "pbiobeBeneficiaryGroup");

				const backgroundBars = beneficiaryGroupEnter
					.append("rect")
				...
				const bars = beneficiaryGroupEnter
					.append("rect")
				...
				const barsPercentage = beneficiaryGroupEnter
					.append("text")
					.attr("class", "pbiobeBarsPercentage")
					.attr("y", function (d) {
						return (
							yScalePercentage(d.beneficiary) +
							yScalePercentage.bandwidth() / 2 +
							percentageVerPadding
						);
					})
				...
				const barsTotalLine1 = beneficiaryGroupEnter
					.append("text")
					...
					.attr("y", function (d) {
						return (
							yScalePercentage(d.beneficiary) +
							yScalePercentage.bandwidth() * 0.4
						);
					})
					.text(0);

				const barsTotalLine2 = beneficiaryGroupEnter
					.append("text")
					...
					.attr("y", function (d) {
						return (
							yScalePercentage(d.beneficiary) +
							yScalePercentage.bandwidth() * 0.8
						);
					})
					.text(0);

				beneficiaryGroup =
					beneficiaryGroupEnter.merge(beneficiaryGroup);
				...
				beneficiaryGroup
					...
					.attr("width", function (d) {
						return (
							xScalePercentage(Math.min(d.percentage, 1)) -
							xScalePercentage(0)
						);
					});

				beneficiaryGroup
					.select(".pbiobeBarsPercentage")
					.transition()
					.duration(duration)
					.attr("x", function (d) {
						return d.percentage < percentageBarLimit
							? xScalePercentage(Math.min(d.percentage, 1)) +
									percentageHorPadding
							: parseInt(formatPercent(d.percentage)) === 100
							? xScalePercentage(Math.min(d.percentage, 1)) +
							  percentageHorNegPadding2
							: d.percentage > 1
							? xScalePercentage(Math.min(d.percentage, 1)) +
							  percentageHorNegPadding3
							: xScalePercentage(Math.min(d.percentage, 1)) +
							  percentageHorNegPadding1;
					})
					.style("fill", function (d) {
						return d.percentage < percentageBarLimit
							? "#666"
					...
					.tween("text", function (d) {
						const node = this;
						const percentage =
							+node.textContent.replace(/\D/g, "") / 100;
						const i = d3.interpolate(
							percentage || 0,
							Math.min(d.percentage, 1)
						);
						return function (t) {
							d3.select(node).text(
								d.percentage > 1
			...
			function createPictogramPanel(data) {
				...
				const title = pictogramsPanel.main
					.selectAll(".pbiobePictogramsPanelTitle")
					.data([true])
					.enter()
					.append("text")
				...
				let pictogramGroupGray = pictogramsPanel.main
					.selectAll(".pbiobePictogramGroupGray")
					.data(data, function (d) {
						return d.beneficiary;
					});

				const pictogramGroupExitGray = pictogramGroupGray
					.exit()
					.remove();

				const pictogramGroupEnterGray = pictogramGroupGray
					.enter()
					.append("g")
					...
					.attr("transform", function (d) {
						...
						return (
							"translate(0," +
							(yScalePictograms(d.beneficiary) -
								pictogramHeight / 2) +
							")"
						);
					});

				const clipPathsGray = pictogramGroupEnterGray
					.append("clipPath")
					.attr("id", function (d) {
				...
				const pathGroupGray = pictogramGroupEnterGray
					.selectAll(".pbiobePathGroupGray")
				...
				const headPathGray = pathGroupGray
					.append("path")
					.attr("d", function (d) {
						return pathAttributes[
							localVariable.get(this)
						].headDAttribute;
					})
					.style("fill", "#ccc");

				const bodyPathGray = pathGroupGray
					.append("path")
					.attr("d", function (d) {
						return pathAttributes[
							localVariable.get(this)
				...
				pictogramGroupGray =
					pictogramGroupEnterGray.merge(pictogramGroupGray);
				...
				pictogramGroupGray
					...
					.attr("width", function (d) {
						return (
							xScalePictograms(d.targeted) - xScalePictograms(0)
						);
					});

				let pictogramGroupBlue = pictogramsPanel.main
					.selectAll(".pbiobePictogramGroupBlue")
					.data(data, function (d) {
						return d.beneficiary;
					});

				const pictogramGroupExitBlue = pictogramGroupBlue
					.exit()
					.remove();

				const pictogramGroupEnterBlue = pictogramGroupBlue
					.enter()
					.append("g")
				...
				const clipPathsBlue = pictogramGroupEnterBlue
					.append("clipPath")
					.attr("id", function (d) {
				...
				const pathGroupBlue = pictogramGroupEnterBlue
					.selectAll(".pbiobePathGroupBlue")
					.data(d3.range(~~(pictogramsPanel.width / pictogramWidth)))
					.enter()
					.append("g")
				...
				const headPathBlue = pathGroupBlue
					.append("path")
					.attr("d", function (d) {
						return pathAttributes[
							localVariable.get(this)
				...
				const bodyPathBlue = pathGroupBlue
					.append("path")
					.attr("d", function (d) {
						return pathAttributes[
							localVariable.get(this)
				...
				pictogramGroupBlue =
					pictogramGroupEnterBlue.merge(pictogramGroupBlue);
				...
				let pictogramRect = pictogramsPanel.main
					.selectAll(".pbiobePictogramRect")
					.data(data, function (d) {
						return d.beneficiary;
					});

				const pictogramRectExit = pictogramRect.exit().remove();

				const pictogramRectEnter = pictogramRect
					.enter()
					.append("rect")
			...
			function createBottomLegend() {
				const legendGroup = bottomGroup
					.selectAll(".pbiobeLegendGroup")
					.data(legendData)
					.enter()
					.append("g")
				...
				const pictogramLegendGroup = bottomGroup
					.selectAll(".pbiobePictogramLegendGroup")
					.data([true])
					.enter()
					.append("g")
				...
				const headPath = pictogramLegendGroup
					.append("path")
				...
				const bodyPath = pictogramLegendGroup
					.append("path")
				...
				let pictogramLegendText = bottomGroup
					.selectAll(".pbiobePictogramLegendText")
					.data([xScalePictograms.domain()[1]]);

				pictogramLegendText = pictogramLegendText
					.enter()
					.append("text")
					...
					.text(function (d) {
						const numberOfPeople = calculateRoundNumber(
							d / ~~(pictogramsPanel.width / pictogramWidth)
						);
						return numberOfPeople
							? ": approx. " + numberOfPeople + " people"
							: ": no people";
					});
			...
			function clickButtonsRects(d, singleSelection) {
				...
				d3.selectAll(".pbiobebuttonsRects").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? unBlue
						: "#eaeaea";
				});

				d3.selectAll(".pbiobebuttonsText").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? "white"
						: "#444";
				});
			...
			function mouseOverGroups(d) {
				...
				svg.selectAll(
					".pbiobeBeneficiaryGroup, .pbiobePictogramGroupGray, .pbiobePictogramGroupBlue"
				...
				const percentageText = d.targeted
					? "(" +
					  (d.beneficiary === "total" ? "people" : d.beneficiary) +
					  " reached: <strong>" +
					  formatPercent2Decimals(d.actual / d.targeted) +
					  "</strong> of the target)"
					: "";
				...
				tooltip
					...
					.html(
						"<strong>" +
							capitalize(d.beneficiary) +
							"</strong><br><div style='margin:8px 0px 8px 0px;display:flex;flex-wrap:wrap;align-items:center;width:" +
							tooltipWidth +
							"px;'><div style='display:flex;margin-bottom:4px;flex:0 65%;white-space:initial;'>Number of " +
							(d.beneficiary === "total"
								? "people"
								: d.beneficiary) +
		...
		function processData(rawData) {
			...
			const data = beneficiariesTypes.map(function (d) {
				const percentage =
					aggregatedData[d + "Targeted"] !== 0
						? aggregatedData[d + "Actual"] /
						  aggregatedData[d + "Targeted"]
		...
		function createAnnotationsDiv() {
			...
			closeRects
				.append("text")
				.attr("class", "pbiobeAnnotationMainText")
				.attr("text-anchor", "middle")
				.attr("x", function (d, i) {
					return (
						width -
		...
		function downloadSnapshotPng(source) {
			...
			const fileName =
				"AffectedPersons_" + csvDateFormat(currentDate) + ".png";
		...
		function downloadSnapshotPdf(source) {
			...
			d3.image(
				...
				pdf.save(
					"AffectedPersons_" + csvDateFormat(currentDate) + ".pdf"
				);
