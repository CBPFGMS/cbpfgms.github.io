...
function calculateRange(node, position) {
	if (position === "top" || position === "bottom")
		return [node.boundaries.left, node.boundaries.right];
	if (position === "right" || position === "left")
		return [node.boundaries.top, node.boundaries.bottom];
}
...
function calculateWaypoints(links, freeRowsAndColumns) {
	...
	links.forEach(link => {
		if (link.needWaypoints) {
			if (link.positions.sourcePosition === "left")
				freeRowsAndColumns.columns[
					link.positions.sourceColumn
				].linkIds.add(link.data.id);
			if (link.positions.sourcePosition === "right")
				freeRowsAndColumns.columns[
					link.positions.sourceColumn + 1
				].linkIds.add(link.data.id);
			if (link.positions.sourcePosition === "top")
				freeRowsAndColumns.rows[link.positions.sourceRow].linkIds.add(
					link.data.id
				);
			if (link.positions.sourcePosition === "bottom")
				freeRowsAndColumns.rows[
					link.positions.sourceRow + 1
				].linkIds.add(link.data.id);
			if (link.positions.targetPosition === "left")
				freeRowsAndColumns.columns[
					link.positions.targetColumn
				].linkIds.add(link.data.id);
			if (link.positions.targetPosition === "right")
				freeRowsAndColumns.columns[
					link.positions.targetColumn + 1
				].linkIds.add(link.data.id);
			if (link.positions.targetPosition === "top")
				freeRowsAndColumns.rows[link.positions.targetRow].linkIds.add(
					link.data.id
				);
			if (link.positions.targetPosition === "bottom")
				freeRowsAndColumns.rows[
					link.positions.targetRow + 1
				].linkIds.add(link.data.id);
			if (
				link.positions.sourceRow !== link.positions.targetRow &&
				link.positions.sourceColumn !== link.positions.targetColumn
			)
				freeRowsAndColumns.columns[
					Math.ceil(
						(link.positions.sourceColumn +
	...
	["rows", "columns"].forEach(type => {
		for (let key in freeRowsAndColumns[type]) {
			freeRowsAndColumns[type][key].scale.domain([
				...freeRowsAndColumns[type][key].linkIds,
			]);
		}
	});
	...
	links.forEach(link => {
		if (link.needWaypoints) {
			...
			const firstWaypoint = {
				x:
					link.positions.sourcePosition === "top" ||
					link.positions.sourcePosition === "bottom"
						? link.sourcePos.x
						: link.positions.sourcePosition === "left"
						? freeRowsAndColumns.columns[
								link.positions.sourceColumn
						  ].scale(link.data.id)
						: freeRowsAndColumns.columns[
				...
				y:
					link.positions.sourcePosition === "left" ||
					link.positions.sourcePosition === "right"
						? link.sourcePos.y
						: link.positions.sourcePosition === "top"
						? freeRowsAndColumns.rows[
								link.positions.sourceRow
						  ].scale(link.data.id)
						: freeRowsAndColumns.rows[
								link.positions.sourceRow + 1
						  ].scale(link.data.id),
			...
			const lastWaypoint = {
				x:
					link.positions.targetPosition === "top" ||
					link.positions.targetPosition === "bottom"
						? link.targetPos.x
						: link.positions.targetPosition === "left"
						? freeRowsAndColumns.columns[
								link.positions.targetColumn
						  ].scale(link.data.id)
						: freeRowsAndColumns.columns[
				...
				y:
					link.positions.targetPosition === "left" ||
					link.positions.targetPosition === "right"
						? link.targetPos.y
						: link.positions.targetPosition === "top"
						? freeRowsAndColumns.rows[
								link.positions.targetRow
						  ].scale(link.data.id)
						: freeRowsAndColumns.rows[
								link.positions.targetRow + 1
						  ].scale(link.data.id),
			...
			if (
				firstWaypoint.x !== lastWaypoint.x &&
				firstWaypoint.y !== lastWaypoint.y
			) {
				const middleWaypoint1 = {
					x: freeRowsAndColumns.columns[
						Math.ceil(
							(link.positions.sourceColumn +
								link.positions.targetColumn) /
								2
			...
			link.waypoints.forEach((waypoint, index) => {
				...
				if (xPrevious === waypoint.x) {
					tempArray.push({
						...
						y:
							waypoint.y +
							Math.sign(yPrevious - waypoint.y) * cornerRadius,
					});
				...
				if (yPrevious === waypoint.y) {
					tempArray.push({
						x:
							waypoint.x +
							Math.sign(xPrevious - waypoint.x) * cornerRadius,
						y: yPrevious,
					});
...
function calculateConnectionPositions({ dataLinksOriginal, links, dataNodes }) {
	dataLinksOriginal.forEach((link, index) => {
		...
		const nodeConnectionSource = dataNodes.find(
			d => d.data.id === link.source
		);
		const nodeConnectionTarget = dataNodes.find(
			d => d.data.id === link.target
		);
		const { column: sourceColumn, row: sourceRow } = nodeSource;
		const { column: targetColumn, row: targetRow } = nodeTarget;

		const [sourcePosition, targetPosition, needWaypoints] = setPositions({
			sourceRow,
			sourceColumn,
			targetRow,
			targetColumn,
	...
	dataNodes.forEach(nodeConnection => {
		connectionPositions.forEach(position => {
			nodeConnection[position].scale.domain(
				nodeConnection[position].linkIds
			);
		});
	});
	...
	links.forEach((link, index) => {
		...
		const nodeConnectionTarget = dataNodes.find(
			d => d.data.id === link.data.target
		);

		const sourcePos = calculateCoordinates(
			link.data.id,
			nodeConnectionSource,
			link.positions.sourcePosition
		);
		const targetPos = calculateCoordinates(
			link.data.id,
			nodeConnectionTarget,
			link.positions.targetPosition
		);
...
function calculateFreeRowsAndColumns(nodes) {
	...
	d3.range(numberOfRows).forEach(row => {
		...
		data.rows[row][1] = d3.min(
			nodesList,
			d => d.boundaries.top - linkPaddingFromNode
		);
		data.rows[row + 1][0] = d3.max(
			nodesList,
			d => d.boundaries.bottom + linkPaddingFromNode
		);
	});
	...
	d3.range(numberOfColumns).forEach(column => {
		...
		data.columns[column][1] = d3.min(
			nodesList,
			d => d.boundaries.left - linkPaddingFromNode
		);
		data.columns[column + 1][0] = d3.max(
			nodesList,
			d => d.boundaries.right + linkPaddingFromNode
		);
	});
	...
	["rows", "columns"].forEach(type => {
		for (let key in data[type]) {
			data[type][key] = {
				...
				count: 0,
				linkIds: new Set(),
				scale: d3.scalePoint().padding(1).range(data[type][key]),
			};
		}
	});
