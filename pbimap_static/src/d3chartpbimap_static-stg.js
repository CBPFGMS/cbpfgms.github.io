(function d3ChartIIFE() {
	const isInternetExplorer =
		...
		cssLinks = [
			...
			"https://cbpfgms.github.io/css/d3chartstylespbimap-stg.css",
			fontAwesomeLink,
			leafletCSSLink,
		],
		...
		html2ToCanvas =
			"https://cbpfgms.github.io/libraries/html2canvasrc3.min.js",
		jsPdf =
	...
	function d3Chart() {
		...
		const width = 1110,
			...
			projectsLogoPath =
				"https://github.com/CBPFGMS/cbpfgms.github.io/raw/master/img/assets/projectslogo.png",
			tooltipThumbnailPath =
				"https://github.com/CBPFGMS/cbpfgms.github.io/raw/master/img/assets/pbimaptooltip.png",
			tooltipThumbnailPathCors =
				"https://raw.githubusercontent.com/CBPFGMS/cbpfgms.github.io/master/img/assets/pbimaptooltip.png",
			partnersLogoPathCors =
				"https://raw.githubusercontent.com/CBPFGMS/cbpfgms.github.io/master/img/assets/partnerslogo.png",
			projectsLogoPathCors =
		...
		const selectedModalityString = queryStringValues.has("modality")
			? queryStringValues.get("modality")
			: null;

		if (selectedModalityString)
			chartState.selectedModality = selectedModalityString.split("|");

		if (selectedResponsiveness === "false") {
			containerDiv.style("width", width + "px");
		}

		const topDiv = containerDiv.append("div").attr("class", "pbimapTopDiv");

		const titleDiv = topDiv.append("div").attr("class", "pbimapTitleDiv");

		const iconsDiv = topDiv
			.append("div")
			.attr("class", "pbimapIconsDiv d3chartIconsDiv");

		const topSvg = containerDiv
			.append("svg")
		...
		const filtersDiv = containerDiv
			.append("div")
			.attr("class", "pbimapFiltersDiv");

		const mapContainedDiv = containerDiv
			.append("div")
			.attr("id", "pbimapContainerDiv")
			.style("height", heightLeafletMap + "px");

		const breadcrumbsDivContainer = containerDiv
			.append("div")
			.style("min-height", "30px")
			.attr("class", "pbimapBreadcrumbDivContainer");

		const breadcrumbDiv = breadcrumbsDivContainer
			.append("div")
			.style("pointer-events", "none")
			.style("position", "absolute")
			.attr("class", "pbimapBreadcrumbDiv");

		const showAllDiv = breadcrumbsDivContainer
			.append("div")
			.style("position", "absolute")
			.style("display", "flex")
			.style("width", "99%")
		...
		const legendDiv = mapContainedDiv
			.append("div")
		...
		if (!isInternetExplorer)
			saveImage(partnersLogoPathCors, "partnersLogo");

		if (!isInternetExplorer)
			saveImage(projectsLogoPathCors, "projectsLogo");

		if (dataStatic) {
			apiFilesStatic.forEach(function (file) {
				promises.push(file + chartState.selectedYear[0] + ".csv");
			});
		} else {
		...
		function receiveInitialData(rawData) {
			...
			const remainingYears = yearsArrayString
				.filter(function (d) {
			...
			function loadDataFile(year) {
				const remainingPromises = dataStatic
					? apiFilesStatic.map(function (file) {
							return d3.csv(file + year + ".csv");
					  })
					: apiFiles.map(function (file) {
							return d3.csv(
		...
		function createTopSvg(data) {
			...
			const previousAllocations = d3
				...
				.size()
				? d3.select(".pbimapTopSvgAllocations").datum()
				: 0;
			const previousBeneficiaries = d3
				.select(".pbimapTopSvgBeneficiaries")
				.size()
			...
			const previousProjects = d3.select(".pbimapTopSvgProjects").size()
				...
				: 0;
			const previousPartners = d3.select(".pbimapTopSvgPartners").size()
				? d3.select(".pbimapTopSvgPartners").datum()
				: 0;

			let topSvgAllocations = topSvg
				.selectAll(".pbimapTopSvgAllocations")
				.data([data.totalAllocations]);

			topSvgAllocations = topSvgAllocations
				.enter()
				.append("text")
			...
			let topSvgAllocationsText = topSvg
				.selectAll(".pbimapTopSvgAllocationsText")
				.data([data.totalAllocations]);

			topSvgAllocationsText = topSvgAllocationsText
				.enter()
				.append("text")
			...
			const topSvgAllocationsSubtitle = topSvg
				.selectAll(".pbimapTopSvgAllocationsSubtitle")
				.data([true])
				.enter()
				.append("text")
			...
			let topSvgPartners = topSvg
				.selectAll(".pbimapTopSvgPartners")
				.data([data.totalPartners]);

			topSvgPartners = topSvgPartners
				.enter()
				.append("text")
			...
			const projectsLogo = topSvg
				.selectAll(".pbimapTopSvgProjectsLogo")
				.data([true])
				.enter()
				.append("image")
			...
			let topSvgProjects = topSvg
				.selectAll(".pbimapTopSvgProjects")
				.data([data.totalProjects]);

			topSvgProjects = topSvgProjects
				.enter()
				.append("text")
		...
		function createFilterDivs(data) {
			...
			const filterContainerDivs = filtersDiv
				.selectAll(null)
				.data(filterTitles)
				.enter()
				.append("div")
			...
			const filterContainerDivsTitles = filterContainerDivs
				.append("div")
			...
			if (isTouchScreenOnly) {
				filterContainerDivs.on("touchstart", function () {
					...
					if (chartState.displayMode === "color") {
						d3MapSvgGroup
							.selectAll(".pbimapColorMarkers")
							.style("opacity", 1);
					} else {
						d3MapSvgGroup
							.selectAll(".pbimapSizeMarkers")
							.style("opacity", 1);
					}
					filterContainerDivs
				...
				d3.select(window).on(
					"touchstart.pbimapFilterContainers",
					function () {
						filterContainerDivs
							.select(".pbimapDropdownContainer")
							.style("display", "none");
			...
			const paddingDiv = filterContainerDivs
				.append("div")
				.attr("class", "pbimapDropdownPaddingDiv");

			const dropdownContainer = filterContainerDivs
				.append("div")
				.attr("class", "pbimapDropdownContainer")
				.style("background-color", function (d) {
					return d3
						.color(filterColorScale(d))
			...
			cbpfsDropdown.selectAll("li").on("click", function (d) {
				if (
					chartState.selectedCBPF.indexOf(d.toLowerCase()) > -1 &&
			...
			partnersDropdown.selectAll("li").on("click", function (d) {
				if (
					chartState.selectedPartner.indexOf(d.toLowerCase()) > -1 &&
			...
			clustersDropdown.selectAll("li").on("click", function (d) {
				if (
					chartState.selectedCluster.indexOf(d.toLowerCase()) > -1 &&
			...
			function changeSelected(datum, thisChartState) {
				if (datum === "All") {
					...
					if (
						thisChartState.length === 1 &&
						thisChartState[0] === "all"
					) {
						thisChartState.length = 0;
					}
			...
			function clickModalities(d) {
				if (
					chartState.selectedModality.indexOf(d.toLowerCase()) > -1 &&
		...
		function createMap(data, fromLegend) {
			if (data.length === 0) {
				...
				const noDataTextBack = d3MapSvgGroup
					.append("text")
				...
				const noDataText = d3MapSvgGroup
					.append("text")
			...
			function createSizeMap() {
				...
				let sizeMapGroup = d3MapSvgGroup
					.selectAll(".pbimapSizeMapGroup")
					.data([true]);
				...
				const sizeMarkersEnter = sizeMarkers
					.enter()
					.append("circle")
				...
				if (isTouchScreenOnly) {
					sizeMarkers.on("touchstart", function (d) {
						d3.event.stopPropagation();
						filtersDiv
							.selectAll(".pbimapDropdownContainer")
							.style("display", "none");
						const self = this;
			...
			function createColorMap() {
				...
				let colorMapGroup = d3MapSvgGroup
					.selectAll(".pbimapColorMapGroup")
					.data([true]);
		...
		function createLegendSvg(data) {
			...
			const buttonsTitle = legendSvg
				.selectAll(".pbimapButtonsTitle")
				.data([true])
				.enter()
				.append("text")
			...
			const legendButtonsGroup = legendSvg
				.selectAll(".pbimapLegendButtonsGroup")
				.data(["size", "color"])
				.enter()
				.append("g")
			...
			legendSvg
				.selectAll(".pbimapLegendButtonsRect")
				.style("fill", function (d) {
					return chartState.displayMode === d ? "#444" : "#ccc";
				});

			legendSvg
				.selectAll(".pbimapLegendButtonsText")
				.style("fill", function (d) {
					return chartState.displayMode !== d ? "#444" : "#ccc";
				});
			...
			let legendSubLegend = legendSvg
				.selectAll(".pbimapLegendSubLegendSquare")
				.data([true]);
			...
			let legendSubLegendText = legendSvg
				.selectAll(".pbimapLegendSubLegendText")
				.data([true]);
			...
			function createSizeLegend() {
				...
				let legendSizeGroups = legendSvg
					.selectAll(".pbimapLegendSizeGroups")
					.data([true]);
				...
				let legendSizeGroup = legendSizeGroups
					.selectAll(".pbimapLegendSizeGroup")
				...
				legendSizeGroup
					.select(".pbimapLegendCirclesText")
					.text(function (d) {
						return d ? d3.formatPrefix(".0", d)(d) : "0";
					});
			...
			function createColorLegend() {
				...
				let legendColorGroup = legendSvg
					.selectAll(".pbimapLegendColorGroup")
					.data([true]);
				...
				const legendColorLines = legendColorGroup
					.selectAll(".pbimapLegendColorLines")
					.data(d3.range(2))
					.enter()
					.append("line")
				...
				let legendColorTexts = legendColorGroup
					.selectAll(".pbimapLegendColorTexts")
					.data(d3.extent(colorScale.domain()));

				legendColorTexts = legendColorTexts
					.enter()
					.append("text")
		...
		function createBreadcrumbDiv() {
			...
			if (chartState.selectedAdminLevel === 0) {
				...
				adminLevel = d3
					.range(1, chartState.selectedAdminLevel + 1)
					.map(function (d) {
			...
			breadcrumbData = breadcrumbData.map(function (d, i) {
				if (d.length > 1) {
					...
					for (var key in partnersList) {
						if (partnersList[key].toLowerCase() === d[0])
							partner = partnersList[key];
					}
			...
			if (breadcrumbData[4] === "All")
				breadcrumbData[4] = "All Allocations";
			...
			let breadcrumbs = breadcrumbDiv
				.selectAll(".pbimapBreadcrumbs")
				.data(breadcrumbData);
			...
			breadcrumbs.select(".pbimapContentSpan").html(function (d) {
				return d;
			});
		...
		function createShowAllButton(data) {
			let showAllButton = showAllDiv
				.selectAll(".pbimapshowAllButton")
		...
		function mouseOverMarker(datum, thisElement) {
			...
			tooltip
				.style("display", "block")
				.html(
					"<div class='pbimapTooltipTitle contributionColorHTMLcolor' style='margin-bottom: -16px;'>" +
						datum.locationName +
						"</div><br><div class='pbimapTooltipSubTitle' style='margin-top: 0px'>" +
						level +
						"</div>" +
						divSpacer +
						thisCBPF +
						"<div style='margin:0px;display:flex;flex-wrap:wrap;width:" +
						tooltipSvgWidth +
						"px;'><div style='display:flex;flex:0 54%;'>Total allocations:</div><div style='display:flex;flex:0 46%;justify-content:flex-end;'><span class='contributionColorHTMLcolor pbimapTooltipTitle2'>$" +
						formatMoney0Decimals(datum.totalAllocation) +
						"</span></div><div style='display:flex;flex:0 54%;'>Number of partners:</div><div style='display:flex;flex:0 46%;justify-content:flex-end;'><span class='contributionColorHTMLcolor pbimapTooltipTitle2'>" +
						datum.numberOfPartners +
						"</span></div><div style='display:flex;flex:0 54%;'>Number of projects:</div><div style='display:flex;flex:0 46%;justify-content:flex-end;'><span class='contributionColorHTMLcolor pbimapTooltipTitle2'>" +
		...
		function generateProjectsList(data, showAll) {
			...
			if (!showAll) {
				const listBreadcrumbDivContainer = listDiv
					.append("div")
					.attr("class", "pbimapListBreadcrumbDivContainer");

				const listBreadcrumbs = listBreadcrumbDivContainer
					.append("p")
					.attr(
						"class",
					...
					.html(function () {
						...
						for (
							let i = 1;
							i <= chartState.selectedAdminLevel;
							i++
						) {
							if (
			...
			const listDownloadButton = listButtonDiv
				.append("button")
				.html(".CSV  ")
				.on("click", function () {
					...
					if (showAll) {
						...
						currentLocation = chartState.selectedAdminLevel
							? data[0]["AdmLoc" + chartState.selectedAdminLevel]
							: cbpfsList[data[0].PFId];
					}
			...
			const listHeaderContainerDiv = listDiv
				.append("div")
				.attr("class", "pbimapListHeaderContainerDiv");

			const headers = listHeaderContainerDiv
				.selectAll(null)
				.data(listHeader)
				.enter()
				.append("div")
		...
		function populateDropdown(selection, dropdownData, thisChartState) {
			let dropdownLi = selection
				.selectAll(".pbimapDropdownLi")
			...
			const dropdownText = dropdownLiEnter
				.append("span")
				.html(function (d) {
					if (thisChartState === chartState.selectedAdminLevel) {
		...
		function processData(summaryData, locationsData) {
			...
			for (let index = locationsData.length; index--; ) {
				...
				if (locationsData[index - 1] || locationsData.length === 1) {
					if (
						...
						thisRow.PrjCode !== locationsData[index + 1].PrjCode
					) {
				...
				thisRow.AllNm =
					"(" +
					cbpfsCodeList[thisRow.PFId] +
					") " +
					foundSummaryObject.AllNm;
			...
			function populateUniqueValues(row) {
				...
				while (--level) {
					row["uniqueValue" + level] =
						row["AdmLoc" + level].split(",").join("") +
						"," +
						row["AdmLocCord" + level];
				}
			}
				...
				for (
					...
					for (let key in thisNextRow) {
						if (thisNextRow[key].indexOf("|||") > -1) {
							thisNextRow[key] =
								thisNextRow[key].split("|||")[numberIndex];
						}
					}
		...
		function filterData() {
			...
			chartState.selectedModality = chartState.selectedModality.filter(
				function (d) {
					return (
						allocationsTypeList
							.map(function (e) {
			...
			if (chartState.selectedAdminLevel === 0) {
				...
				nestedData.forEach(function (row) {
					...
					row.values.forEach(function (valueRow) {
						...
						if (numberOfProjects.indexOf(valueRow.PrjCode) === -1)
							numberOfProjects.push(valueRow.PrjCode);
					});
				...
				nestedData.forEach(function (row) {
					...
					row.values.forEach(function (valueRow) {
						totalAllocation +=
							valueRow[
								"AdmLocClustBdg" + chartState.selectedAdminLevel
							];
						beneficiariesList.forEach(function (d) {
							numberOfBeneficiaries[d] +=
								valueRow[
						...
						if (numberOfProjects.indexOf(valueRow.PrjCode) === -1)
							numberOfProjects.push(valueRow.PrjCode);
						if (
							totalNumberOfPartners.indexOf(valueRow.PartCode) ===
							-1
						)
							totalNumberOfPartners.push(valueRow.PartCode);
						if (
							totalNumberOfProjects.indexOf(valueRow.PrjCode) ===
							-1
						)
							totalNumberOfProjects.push(valueRow.PrjCode);
					});
		...
		function createCountriesCoordinates(cbpfsData) {
			cbpfsData.forEach(function (row) {
				countriesCoordinates[row.PFName + ""] =
					row.PFLat + "," + row.PFLong;
			});
		}

		function getDataAttributes() {
			dataAttributes.forEach(function (attribute) {
				const queryStringName =
					attribute === "CBPF" ? "fund" : attribute.toLowerCase();

				const attibuteValues = queryStringValues.has(queryStringName)
					? queryStringValues
							.get(queryStringName)
							.replace(/\|/g, ",")
							.split(",")
					...
					: containerDiv
							.node()
							.getAttribute("data-" + attribute.toLowerCase())
							.split(",")
							.map(function (d) {
								return d.trim().toLowerCase();
							});
				...
				const someInvalidValue = chartState[
					"selected" + attribute
				...
				if (someInvalidValue)
					chartState["selected" + attribute] = ["all"];
			});

			chartState.selectedAdminLevel = queryStringValues.has("adminlevel")
				? ~~queryStringValues.get("adminlevel") < 7
		...
		function validateYear(yearString) {
			return +yearString === +yearString &&
				yearsArrayString.indexOf(yearString) > -1
				? +yearString
				: currentYear;
		}
		...
		function createAnnotationsDiv() {
			...
			const breadcrumbsDivSize = breadcrumbsDivContainer
				.node()
				.getBoundingClientRect();

			const topSvgHeight = topSvgSize.height * (width / topSvgSize.width);

			const filtersDivHeight =
				filtersDivSize.height * (width / filtersDivSize.width);

			const mapHeight = mapSize.height * (width / mapSize.width);

			const breadcrumbsHeight =
				breadcrumbsDivSize.height * (width / breadcrumbsDivSize.width);
			...
			const realHeight =
				topDivSize.height +
				topSvgSize.height +
				filtersDivSize.height +
				mapSize.height +
				breadcrumbsDivSize.height;
			...
			const tooltipThumbnail = helpSVG
				.append("image")
				.attr(
					"xlink:href",
		...
		function createCsv(data) {
			...
			data.forEach(function (d) {
				const country = chartState.selectedAdminLevel
					? d.CBPFName
					: d.locationName;
				csvData.push({
					"CBPF Name": country,
					...
					"Total Allocations":
						Math.floor(d.totalAllocation * 100) / 100,
		...
		function createSnapshot(type, fromContextMenu) {
			...
			html2canvas(imageDiv, {
				...
				topSvg.attr("width", null).attr("height", null);
		...
		function downloadSnapshotPng(source) {
			...
			const fileName =
				"AllocationsOverview_" + csvDateFormat(currentDate) + ".png";
		...
		function downloadSnapshotPdf(source) {
			...
			d3.image(
				...
				const intro = pdf.splitTextToSize(
					"Funding from CBPFs is directly available to UN agencies, national and international non-governmental organizations (NGOs) and Red Cross/ Red Crescent organizations. In 2018, CBPFs allocated more than $836 million to 685 partners in 18 countries to support 1,453 critical humanitarian projects. These projects targeted millions of people with healthcare, food aid, clean water, shelter and other life-saving assistance.",
				...
				const yearsPdfList = chartState.selectedYear
					.sort(function (a, b) {
						return a - b;
					})
				...
				const partnersPdfList =
					chartState.selectedPartner[0] === "all"
						? "All Partners"
						: createPdfList(
								chartState.selectedPartner,
				...
				const clusterPdfList =
					chartState.selectedCluster[0] === "all"
						? "All Clusters"
						: createPdfList(
								chartState.selectedCluster,
				...
				const allocationPdfList =
					chartState.selectedModality[0] === "all"
						? "All Allocations"
						: createPdfList(
								chartState.selectedModality,
								allocationsTypeList
				...
				const locationPdfList = !chartState.selectedAdminLevel
					? "Global Level"
					: "Admin Level " + chartState.selectedAdminLevel;
				...
				pdf.fromHTML(
					"<div style='margin-bottom: 2px; font-family: Arial, sans-serif; color: rgb(60, 60 60);'>Date: <span style='color: rgb(65, 143, 222); font-weight: 700;'>" +
						...
						"<span style='color: rgb(65, 143, 222); font-weight: 700;'>" +
						allocationPdfList +
						"</span></div><div style='font-family: Arial, sans-serif; color: rgb(60, 60 60);'>" +
						locationText +
						"<span style='color: rgb(65, 143, 222); font-weight: 700;'>" +
						locationPdfList +
						"</span></div>",
