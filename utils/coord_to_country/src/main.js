...
function handleLookup() {
	...
	const countryData = getCountryFromCoords(latVal, lngVal);

	if (countryData) {
		resultElement.textContent = `Country: ${countryData.name} (${countryData.code})`;
	} else {
		resultElement.textContent =
			"Location not found or in international waters.";
	}
}

// 1. Trigger when clicking the button
findBtn.addEventListener("click", handleLookup);

// 2. Trigger when hitting 'Enter' inside the form fields, while blocking web reload
...
function getCountryFromCoords(lat, lng) {
	try {
		// CRITICAL: country-coder expects [Longitude, Latitude] formatting (Lng/Lat order)
		const isoCode = iso1A2Code([lng, lat]);
		if (!isoCode) {
			return null;
		}
		const fullName = countryCodes[isoCode] || `Country Code: ${isoCode}`;
		return { name: fullName, code: isoCode };
	} catch (error) {
		console.error("Error during client-side geocoding lookup:", error);
		return null;
	}
}
