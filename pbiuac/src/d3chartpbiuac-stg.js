(function d3ChartIIFE() {
	const isInternetExplorer =
		...
		cssLinks = [
			...
			"https://cbpfgms.github.io/css/d3chartstylespbiuac-stg.css",
			fontAwesomeLink,
		],
	...
	function d3Chart() {
		...
		const width = 900,
			...
			helpPortalUrl =
				"https://gms.unocha.org/content/allocations-timeline",
			...
			colorInterpolatorStandard = d3.interpolateRgb(
				d3.color(rectangleColors[0]).brighter(1.2),
				d3.color(rectangleColors[0]).darker(1.2)
			),
			colorInterpolatorReserve = d3.interpolateRgb(
				d3.color(rectangleColors[1]).brighter(1.2),
				d3.color(rectangleColors[1]).darker(1.2)
			),
		...
		const topDiv = containerDiv.append("div").attr("class", "pbiuacTopDiv");

		const titleDiv = topDiv.append("div").attr("class", "pbiuacTitleDiv");
		...
		const footerDiv = !isPfbiSite
			? containerDiv.append("div").attr("class", "pbiuacFooterDiv")
			: null;
		...
		const xScaleMain = d3
			...
			.range([
				mainPanel.padding[3],
				mainPanel.width - mainPanel.padding[1],
			]);

		const xScaleBrush = d3
			.scaleTime()
			.range([
				brushPanel.padding[3],
				brushPanel.width - brushPanel.padding[1],
			]);
		...
		const colorScaleStandard = d3.scaleQuantile().range(colorsStandard);

		const colorScaleReserve = d3.scaleQuantile().range(colorsReserve);

		const legendVerticalScale = d3
			.scalePoint()
			.domain(allocationTypes)
			.range([
				legendPanel.padding[0],
				legendPanel.height - legendPanel.padding[2],
			])
			.padding(0.75);

		const yAxis = d3.axisLeft(yScaleMain).tickSize(3);
		...
		const mainClip = mainPanel.main
			...
			.attr(
				"width",
				mainPanel.width - mainPanel.padding[3] - mainPanel.padding[1]
			);
			...
			.on("end", function () {
				if (d3.event.selection) {
					...
					brushPanel.main
						...
						.call(brush.move, xScaleBrush.range());
				}
			});
			...
			yScaleBrush
				...
				.range(
					setOrdinalRange(
						brushPanel,
						barHeightBrush,
						outerBarPaddingBrush,
						innerBarPaddingBrush
					)
				);
		...
		function createTopPanel(completeData) {
			const data = completeData.filter(function (d) {
				return (
					d.PlannedStartDateTimestamp <
						xScaleMain.domain()[1].getTime() &&
					d.PlannedEndDateTimestamp >
						xScaleMain.domain()[0].getTime() &&
					(chartState.selectedYear[0] === allYearsOption
						? true
						: chartState.selectedYear.indexOf(+d.AllocationYear) >
						  -1)
				);
			});
			...
			const topPanelMoneyBag = topPanel.main
				.selectAll(".pbiuactopPanelMoneyBag")
				.data([true])
				.enter()
				.append("g")
			...
			const previousValue =
				d3.select(".pbiuactopPanelMainValue").size() !== 0
					? d3.select(".pbiuactopPanelMainValue").datum()
					: 0;

			const previousUpcoming =
				d3.select(".pbiuactopPanelUpcomingNumber").size() !== 0
					? d3.select(".pbiuactopPanelUpcomingNumber").datum()
					: 0;

			const previousOngoing =
				d3.select(".pbiuactopPanelOngoingNumber").size() !== 0
					? d3.select(".pbiuactopPanelOngoingNumber").datum()
					: 0;

			const previousPast =
				d3.select(".pbiuactopPanelPastNumber").size() !== 0
					? d3.select(".pbiuactopPanelPastNumber").datum()
					: 0;

			let mainValueGroup = topPanel.main
				.selectAll(".pbiuacmainValueGroup")
				.data([true]);
			...
			let topPanelMainValue = mainValueGroup
				.selectAll(".pbiuactopPanelMainValue")
				.data([mainValue]);
			...
			let topPanelMainText = mainValueGroup
				.selectAll(".pbiuactopPanelMainText")
				.data([mainValue]);
			...
			let topPanelSubText = mainValueGroup
				.selectAll(".pbiuactopPanelSubText")
				.data([true]);
			...
			let topPanelUpcomingNumber = mainValueGroup
				.selectAll(".pbiuactopPanelUpcomingNumber")
				.data([upcomingValue]);

			topPanelUpcomingNumber = topPanelUpcomingNumber
				.enter()
				.append("text")
				.attr(
					"class",
					"pbiuactopPanelUpcomingNumber contributionColorFill"
				)
				.attr("text-anchor", "end")
			...
			let topPanelUpcomingText = mainValueGroup
				.selectAll(".pbiuactopPanelUpcomingText")
				.data([true]);

			topPanelUpcomingText = topPanelUpcomingText
				.enter()
				.append("text")
			...
			let topPanelUpcomingTextSubText = mainValueGroup
				.selectAll(".pbiuactopPanelUpcomingTextSubText")
				.data([true]);

			topPanelUpcomingTextSubText = topPanelUpcomingTextSubText
				.enter()
				.append("text")
			...
			let topPanelOngoingNumber = mainValueGroup
				.selectAll(".pbiuactopPanelOngoingNumber")
				.data([ongoingValue]);

			topPanelOngoingNumber = topPanelOngoingNumber
				.enter()
				.append("text")
				.attr(
					"class",
					"pbiuactopPanelOngoingNumber contributionColorFill"
				)
				.attr("text-anchor", "end")
			...
			let topPanelOngoingText = mainValueGroup
				.selectAll(".pbiuactopPanelOngoingText")
				.data([true]);

			topPanelOngoingText = topPanelOngoingText
				.enter()
				.append("text")
			...
			let topPanelOngoingTextSubText = mainValueGroup
				.selectAll(".pbiuactopPanelOngoingTextSubText")
				.data([true]);

			topPanelOngoingTextSubText = topPanelOngoingTextSubText
				.enter()
				.append("text")
			...
			let topPanelPastNumber = mainValueGroup
				.selectAll(".pbiuactopPanelPastNumber")
				.data([pastValue]);

			topPanelPastNumber = topPanelPastNumber
				.enter()
				.append("text")
			...
			let topPanelPastText = mainValueGroup
				.selectAll(".pbiuactopPanelPastText")
				.data([true]);

			topPanelPastText = topPanelPastText
				.enter()
				.append("text")
			...
			let topPanelPastTextSubText = mainValueGroup
				.selectAll(".pbiuactopPanelPastTextSubText")
				.data([true]);

			topPanelPastTextSubText = topPanelPastTextSubText
				.enter()
				.append("text")
		...
		function createButtonsPanel(data) {
			...
			function clickButtonsRects(d, singleSelection) {
				...
				d3.selectAll(".pbiuacbuttonsRects").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? unBlue
						: "#eaeaea";
				});

				d3.selectAll(".pbiuacbuttonsText").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? "white"
						: "#444";
				});
				...
				brushPanel.main
					.select(".pbiuacBrushGroup")
					.call(brush.move, [
						xScaleBrush(offsetStartDate),
						xScaleBrush(offsetEndDate),
					]);
		...
		function createMainPanel(data) {
			const backgroundRects = mainPanel.main
				.selectAll(null)
				.data(cbpfsList)
				.enter()
				.append("rect")
			...
			const yAxisGroup = mainPanel.main
				.append("g")
				.attr("class", "pbiuacYAxisGroup")
				.attr("transform", "translate(" + mainPanel.padding[3] + ",0)")
				.call(yAxis);

			yAxisGroup.selectAll("text").each(function (d, i) {
				d3.select(this).attr(
					"transform",
					"translate(0," +
						(yScaleMain.range()[i + 1] - yScaleMain(d)) / 2 +
						")"
				);
			});

			yAxisGroup.selectAll("line").each(function (d, i) {
				d3.select(this).attr(
					"transform",
					"translate(0," +
						(yScaleMain.range()[i + 1] - yScaleMain(d)) / 2 +
			...
			const xAxisMainGroup = mainPanel.main
				.append("g")
				.attr("class", "pbiuacXAxisMainGroup")
				.attr("transform", "translate(0," + mainPanel.padding[0] + ")")
				.call(xAxisMain);

			const todayContainerGroup = mainPanel.main
				.append("g")
				.attr("clip-path", "url(#pbiuacclip)");

			const todayGroup = todayContainerGroup
				.append("g")
				.attr("class", "pbiuacTodayGroup")
				.attr(
					"transform",
					"translate(" + xScaleMain(currentDate) + ",0)"
				);
			...
			const allocationsMain = mainPanel.main
				.selectAll(null)
				.data(data)
				.enter()
				.append("rect")
				...
				.attr("y", function (d) {
					return (
						yScaleMain(d.PooledFundName) +
						outerBarPadding / 2 +
						d.row * barHeight +
						(d.row + 1) * innerBarPadding
					);
				})
			...
			function mouseOverAllocation(datum, element) {
				...
				const thisColor =
					datum.allocationType === "standard"
						? "contributionColorHTMLcolor"
						: "allocationColorHTMLcolor";

				innerTooltip.html(
					"<div class='pbiuactooltipTitleDiv'><div class='" +
						thisColor +
						"'><b>" +
						datum.AllocationTitle +
						"</b></div><div class='pbiuaccloseDiv'><span class='fas fa-times'></span></div></div><div class='pbiuacSpacer'></div>CBPF: <b>" +
						datum.PooledFundName +
						"</b><br>Amount: $<b>" +
						formatMoney0Decimals(datum.TotalUSDPlanned) +
						"</b><div class='pbiuacSpacer'></div>Start Date: " +
						timeFormat(datum.PlannedStartDate) +
						"<br>End Date: " +
						timeFormat(datum.PlannedEndDate) +
						"<div class='pbiuacSpacer'></div><div class='pbiuacTooltipButtonDiv' style='height:30px;display:flex;justify-content:center;align-items:center;'><button>Display Details</button></div>"
				);
				...
				tooltip.select(".pbiuaccloseDiv").on("click", function () {
						...
						mouse[0] >
							...
							: mouse[0] <
							  tooltipSize.width / 2 +
									mainPanel.padding[3] +
									padding[0]
							? mainPanel.padding[3] + padding[0] + "px"
							: mouse[0] - tooltipSize.width / 2 + "px"
					)
					.style(
						"top",
						mouse[1] < tooltipSize.height
							? thisSize.top -
									containerSize.top +
									thisSize.height +
									1 +
		...
		function createBrushPanel(data) {
			const xAxisBrushGroup = brushPanel.main
				.append("g")
				.attr("class", "pbiuacXAxisBrushGroup")
				.attr("transform", "translate(0," + brushPanel.padding[0] + ")")
				.call(xAxisBrush);

			const todayGroupBrush = brushPanel.main
				.append("g")
				.attr(
					"transform",
					"translate(" + xScaleBrush(currentDate) + ",0)"
				);

			const todayLineBrush = todayGroupBrush
				.append("line")
			...
			const todayTextBrush = todayGroupBrush
				.append("text")
				.attr("text-anchor", "middle")
				.attr("font-family", "Arial")
				.attr("font-size", "11px")
				...
				.attr(
					"y",
					brushPanel.padding[0] -
						xAxisBrush.tickPadding() -
						xAxisBrush.tickSizeInner()
				)
				.text("Today");

			const todaySizeBrush = todayTextBrush
				.node()
				.getBoundingClientRect();

			xAxisBrushGroup.selectAll("text").each(function () {
				const thisSize = this.getBoundingClientRect();
				if (
					thisSize.left < todaySizeBrush.right &&
					thisSize.right > todaySizeBrush.left
				) {
				...
				if (+thisText === +thisText) {
					d3.select(this)
						.style("fill", axisHighlightColor)
						.style("font-weight", 700);
				}
			});

			const allocationsBrush = brushPanel.main
				.selectAll(null)
				.data(data)
				.enter()
				.append("rect")
				...
				.attr("width", function (d) {
					return (
						xScaleBrush(d.PlannedEndDate) -
						xScaleBrush(d.PlannedStartDate)
					);
				})
				.attr("y", function (d) {
					return (
						yScaleBrush(d.PooledFundName) +
						outerBarPaddingBrush / 2 +
						d.row * barHeightBrush +
						(d.row + 1) * innerBarPaddingBrush
					);
				})
			...
			const brushGroup = brushPanel.main
				.append("g")
			...
			const brushText = brushPanel.main
				.append("text")
				.attr("text-anchor", "end")
			...
			const brushPanelLine = brushPanel.main
				.append("line")
		...
		function filterAllocations(data) {
			mainPanel.main
				.selectAll(".pbiuacAllocationsMain")
				.style("fill", function (d) {
					if (d.allocationType === "standard") {
						return colorScaleStandard(d.TotalUSDPlanned);
					} else {
				...
				.style("display", function (d) {
					return chartState.selectedYear.indexOf(+d.AllocationYear) >
						-1 || chartState.selectedYear[0] === allYearsOption
						? null
						: "none";
				});

			brushPanel.main
				.selectAll(".pbiuacAllocationsBrush")
				.style("fill", function (d) {
					if (d.allocationType === "standard") {
						return colorScaleStandard(d.TotalUSDPlanned);
					} else {
		...
		function createLegendPanel() {
			const legendGroups = legendPanel.main
				.selectAll(null)
			...
			const standardRectangles = legendPanel.main
				.selectAll(null)
				.data(colorsStandard)
				.enter()
				.append("rect")
				.attr(
					"y",
					legendVerticalScale("standard") - legendRectHeight / 2
				)
				.attr("x", function (_, i) {
			...
			const reserveRectangles = legendPanel.main
				.selectAll(null)
				.data(colorsReserve)
				.enter()
				.append("rect")
				.attr(
					"y",
					legendVerticalScale("reserve") - legendRectHeight / 2
				)
				.attr("x", function (_, i) {
					return legendScalePadding + i * legendRectWidth;
				})
			...
			minValueStandard = legendPanel.main
				.append("text")
			...
			maxValueStandard = legendPanel.main
				.append("text")
				.attr("class", "pbiuacLegendMainText2")
				.attr("y", legendVerticalScale("standard") + 3)
				.attr(
					"x",
					legendScalePadding +
						colorsStandard.length * legendRectWidth +
						4
				)
				.attr("text-anchor", "start");

			minValueReserve = legendPanel.main
				.append("text")
				.attr("class", "pbiuacLegendMainText2")
			...
			maxValueReserve = legendPanel.main
				.append("text")
				.attr("class", "pbiuacLegendMainText2")
				.attr("y", legendVerticalScale("reserve") + 3)
				.attr(
					"x",
					legendScalePadding +
						colorsReserve.length * legendRectWidth +
						4
				)
				.attr("text-anchor", "start");
			...
			function mouseOverLegend(datum, scale, element, modality) {
				...
				mainPanel.main
					...
					.style("opacity", function (d) {
						return d.TotalUSDPlanned >= limits[0] &&
							d.TotalUSDPlanned <= limits[1] &&
							d.AllocationSource === modality
							? 1
							: 0.1;
					});
				...
				innerTooltip.html(
					"This color represents values going from:<br><div style='margin-top:8px;text-align:center;'>$<strong>" +
						formatMoney0Decimals(limits[0]) +
						"</strong> to $<strong>" +
						formatMoney0Decimals(limits[1]) +
						"</strong></div>"
				);
		...
		function brushed() {
			...
			const queryStartDate =
				d3.timeDay.count(currentDate, xScaleMain.domain()[0]) * -1;
			const queryEndDate = d3.timeDay.count(
				currentDate,
				xScaleMain.domain()[1]
			);
			mainPanel.main
			...
			mainPanel.main
				.select(".pbiuacTodayGroup")
				.attr(
					"transform",
					"translate(" + xScaleMain(currentDate) + ",0)"
				);
			mainPanel.main.select(".pbiuacXAxisMainGroup").call(xAxisMain);
			rewriteXAxisText();
			createTopPanel(completeData);
			mainPanel.main
				.select(".pbiuacZoomRect")
				.call(
					zoom.transform,
					d3.zoomIdentity
						.scale(
		...
		function zoomed() {
			...
			brushPanel.main
				.select(".pbiuacBrushGroup")
				.call(brush.move, xScaleMain.range().map(t.invertX, t));
		}

		function preProcessData(rawData) {
			rawData.forEach(function (row) {
				if (yearsArray.indexOf(+row.AllocationYear) === -1)
					yearsArray.push(+row.AllocationYear);
			});
		...
		function calculateDomain(data) {
			if (chartState.selectedYear[0] === allYearsOption) {
				...
				data.forEach(function (row) {
					if (
						chartState.selectedYear.indexOf(+row.AllocationYear) >
						-1
					) {
						if (
							thisMinDate.getTime() >
							row.PlannedStartDateTimestamp
						)
							thisMinDate = row.PlannedStartDate;
						if (thisMaxDate.getTime() < row.PlannedEndDateTimestamp)
							thisMaxDate = row.PlannedEndDate;
					}
				});
				offsetStartDate = d3.timeMonth.offset(
					thisMinDate,
					-offsetStartDefault
				);
				offsetEndDate = d3.timeMonth.offset(
					thisMaxDate,
					offsetEndDefault
				);
			}
		...
		function setColorScale(data) {
			...
			const colorArray = data.reduce(
				function (acc, curr) {
					if (
						chartState.selectedYear[0] === allYearsOption ||
						chartState.selectedYear.indexOf(+curr.AllocationYear) >
							-1
					) {
						acc[curr.AllocationSource.toLowerCase()].push(
							+curr.TotalUSDPlanned
						);
						if (curr.allocationType === "standard") {
							if (
								+curr.TotalUSDPlanned <
								minAllocationValueStandard
							)
								minAllocationValueStandard =
									+curr.TotalUSDPlanned;
							if (
								+curr.TotalUSDPlanned >
								maxAllocationValueStandard
							)
								maxAllocationValueStandard =
									+curr.TotalUSDPlanned;
						} else {
							if (
								+curr.TotalUSDPlanned <
								minAllocationValueReserve
							)
								minAllocationValueReserve =
									+curr.TotalUSDPlanned;
							if (
								+curr.TotalUSDPlanned >
								maxAllocationValueReserve
							)
								maxAllocationValueReserve =
									+curr.TotalUSDPlanned;
						}
			...
			minValueStandard.text(
				"Minimum Amount: $" +
					formatMoney0Decimals(minAllocationValueStandard)
			);
			maxValueStandard.text(
				"Maximum Amount: $" +
					formatMoney0Decimals(maxAllocationValueStandard)
			);
			minValueReserve.text(
				"Minimum Amount: $" +
					formatMoney0Decimals(minAllocationValueReserve)
			);
			maxValueReserve.text(
				"Maximum Amount: $" +
					formatMoney0Decimals(maxAllocationValueReserve)
			);
		}

		function processData(rawData) {
			rawData.forEach(function (row) {
				const thisAllocationTime =
					row.PlannedEndDateTimestamp < currentDate.getTime()
						? "past"
						: row.PlannedStartDateTimestamp > currentDate.getTime()
						? "planned"
						: "ongoing";
				...
				if (cbpfsAllocationsTime[row.PooledFundName] === undefined) {
					cbpfsAllocationsTime[row.PooledFundName] =
						thisAllocationTime;
				} else if (
					cbpfsOrdering.indexOf(
						cbpfsAllocationsTime[row.PooledFundName]
					) > cbpfsOrdering.indexOf(thisAllocationTime)
				) {
					cbpfsAllocationsTime[row.PooledFundName] =
						thisAllocationTime;
				}

				if (minDate.getTime() > row.PlannedStartDateTimestamp)
					minDate = row.PlannedStartDate;
				if (maxDate.getTime() < row.PlannedEndDateTimestamp)
					maxDate = row.PlannedEndDate;
			});

			const cbpfsUpcoming = Object.keys(cbpfsAllocationsTime)
				.filter(function (d) {
			...
			const cbpfsOngoing = Object.keys(cbpfsAllocationsTime)
				.filter(function (d) {
			...
			const cbpfsPast = Object.keys(cbpfsAllocationsTime)
				.filter(function (d) {
		...
		function calculateCollisions(cbpf, cbpfName) {
			cbpf = cbpf.sort(function (a, b) {
				return (
					a.PlannedStartDateTimestamp - b.PlannedStartDateTimestamp
				);
			});
			...
			cbpf.forEach(function (d, index) {
				...
				const maxKey = d3.max(
					Object.keys(rowsList).map(function (e) {
						return +e;
					})
				);
		...
		function calculateHeightandResize() {
			const mainPanelInnerHeight = Object.keys(cbpfsRowList).reduce(
				function (acc, curr) {
					acc +=
						2 * outerBarPadding +
						cbpfsRowList[curr] * innerBarPadding +
						(cbpfsRowList[curr] + 1) * barHeight;
					return acc;
				},
				0
			);
			...
			mainPanelHeight =
				mainPanelInnerHeight +
				mainPanel.padding[0] +
				mainPanel.padding[2];

			mainPanel.height = mainPanelHeight;

			const brushPanelInnerHeight = Object.keys(cbpfsRowList).reduce(
				function (acc, curr) {
					acc +=
						2 * outerBarPaddingBrush +
						cbpfsRowList[curr] * innerBarPaddingBrush +
						(cbpfsRowList[curr] + 1) * barHeightBrush;
					return acc;
				},
				0
			);
		...
		function setOrdinalRange(
			panel,
			barHeight,
			outerBarPadding,
			innerBarPadding
		) {
			let range = Object.keys(cbpfsRowList).map(
				function (d) {
					return (this.current +=
						2 * outerBarPadding +
						cbpfsRowList[d] * innerBarPadding +
						(cbpfsRowList[d] + 1) * barHeight);
				},
				{
		...
		function rewriteXAxisText() {
			const todaySize = mainPanel.main
				.select(".pbiuacTodayText")
				.node()
				.getBoundingClientRect();
			mainPanel.main
				.select(".pbiuacXAxisMainGroup")
				.selectAll("text")
				.each(function () {
					const thisSize = this.getBoundingClientRect();
					if (
						thisSize.left < todaySize.right &&
						thisSize.right > todaySize.left
					) {
						d3.select(this.parentNode).remove();
						return;
					}
		...
		function row(d) {
			...
			d.PlannedStartDate =
				timeParse(d.PlannedStartDate) || new Date(d.PlannedStartDate);
			d.PlannedEndDate =
				timeParse(d.PlannedEndDate) || new Date(d.PlannedEndDate);
			if (!d.PlannedStartDate && !d.PlannedEndDate) return;
			if (!d.PlannedStartDate) {
				d.PlannedStartDate =
					d.AllocationSource === "Standard"
						? d3.timeMonth.offset(d.PlannedEndDate, -1)
						: d3.timeDay.offset(d.PlannedEndDate, -15);
			}
			if (!d.PlannedEndDate) {
				d.PlannedEndDate =
					d.AllocationSource === "Standard"
						? d3.timeMonth.offset(d.PlannedStartDate, 1)
						: d3.timeDay.offset(d.PlannedStartDate, 15);
			}
		...
		function generateList(datum) {
			...
			const allocationStart = listDiv
				...
				.html(
					"<span class='" +
						thisColorClass +
						"'>Allocation strategy launch date: </span>" +
						timeFormat(datum.PlannedStartDate)
				);

			const allocationEnd = listDiv
				.append("div")
				...
				.html(
					"<span class='" +
						thisColorClass +
						"'>Expected completion date of HC approvals: </span>" +
						timeFormat(datum.PlannedEndDate)
				);

			const thisSummary = datum.AllocationSummary
				? datum.AllocationSummary
				: "n/a";

			const allocationSummary = listDiv
				.append("div")
				.attr("class", "pbiuacAllocationEnd")
				.append("p")
				.html(
					"<span class='" +
						thisColorClass +
						"'>Allocation Statement: </span>" +
						thisSummary
				);

			const hrList = datum.HRPPlans
				? datum.HRPPlans.split("##").join("; ") + "."
				: "n/a.";

			const allocationHR = listDiv
				.append("div")
			...
			const documentsDiv = listDiv
				.append("div")
				.attr("class", "pbiuacDocumentsDiv")
				.append("p")
				.html("<b>Related Documents:</b>");

			const documentsList = datum.Documents
				? datum.Documents.split("|||").map(function (d) {
						return {
			...
			const documentsDivList = listDiv
				.append("div")
			...
			if (documentsList) {
				...
				const rows = list
					...
					.enter()
					.append("li");
				rows.append("span").html(function (d) {
		...
		function createCsv(data) {
			const copiedData = JSON.parse(JSON.stringify(data)).filter(
				function (d) {
					return (
						d.PlannedStartDateTimestamp <
							xScaleMain.domain()[1].getTime() &&
			...
			const helpData = [
				{
					...
					xTooltip:
						(padding[3] + buttonsPanel.padding[3] - 4) *
				...
				},
				{
					...
					height: 44,
					xTooltip:
						(padding[3] + legendPanel.padding[3] - 4) *
						(topDivSize.width / width),
					yTooltip:
						(topDivHeight +
							height -
		...
		function downloadSnapshotPng(source) {
			...
			const fileName =
				"AllocationsTimeline_" + csvDateFormat(currentDate) + ".png";
		...
		function downloadSnapshotPdf(source) {
			...
			d3.image(
				...
				const intro = pdf.splitTextToSize(
					"In 2018, CBPFs allocated more than $836 million to 685 partners in 18 countries to support 1,453 critical humanitarian projects. These projects targeted millions of people with healthcare, food aid, clean water, shelter and other life-saving assistance.",
				...
				pdf.save(
					"AllocationsTimeline_" + csvDateFormat(currentDate) + ".pdf"
				);
		...
		function validateYear(yearString) {
			if (
				yearString.toLowerCase() === allYearsOption ||
				yearString === allYearsOption
			) {
