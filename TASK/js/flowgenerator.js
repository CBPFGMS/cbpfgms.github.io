...
function flowNodesGenerator({ dataNodesOriginal, width, numberOfColumns, svg }) {
	...
	yScale.range([padding[0], variables.height - padding[2]])
	...
	const invisibleGroup = svg.append("g")
		.style("opacity", 0)
	...
	dataNodesOriginal.forEach((node, index) => {
		...
		const fakeText = invisibleGroup.append("text")
		...
		if (lines === 1) {
			...
			const choppedText = fakeTextLength > maxTextSize ?
				chopText(fakeText, maxTextSize) : node.text;
			textArray = breakStringInTwo(choppedText);
			const fakeTextTwoLines = invisibleGroup.append("text")
				.attr("class", classPrefix + "nodeText")
		...
		nodes[index] = {
			...
			rectWidth: Math.max(minRectWidth, Math.min(maxRectWidth, textBox.width + (2 * nodesTextPaddingHorizontal))),
